...
import { Search, ArrowLeft, Heart, Bell, ChevronDown, ChevronRight, Star, TrendingUp, Users, Award, Music, Palette, Video, Camera, MessageCircle, X } from 'lucide-react';
...
const CreatorDiscoveryPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        extractCategoriesFromCreators(newList);
        
        // Fetch follower counts for each creator
        fetchFollowerCounts(newList);
      } catch (error) {
  ...
  // Fetch follower counts for each creator
  const fetchFollowerCounts = async (creators: Creator[]) => {
      ...
      await Promise.all(
        creators.map(async (creator) => {
          if (creator.vendor && creator.vendor.id) {
            console.log("Fetching followers for vendor ID:", creator.vendor.id);
  
            const result = await retriveVendorsFollowers(creator.vendor.id);
  
            console.log(`Followers fetched for ${creator.vendor.id}:`, result.count);
  
            counts[creator.vendor.id.trim()] = typeof result.count === 'number' ? result.count : 0;
          }
        })
      );
  ...
  //           // Expecting result to be an array of followers
  //           if (Array.isArray(result)) {
  //             counts[creator.vendor.id] = result.length;
  //           } else {
  ...
  const extractCategoriesFromCreators = (creators: Creator[]) => {
    ...
    creators.forEach(creator => {
      if (creator.vendor.creator_title && creator.vendor.creator_title.trim()) {
  ...
  useEffect(() => {
    ...
    const filtered = customerVendors.filter(creator => {
      ...
      const matchesSearch = searchQuery === '' || 
                          (creator.vendor.name?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false);
      
      // Filter by category
      const matchesCategory = activeCategory === 'all' || 
                             (creator.vendor.creator_title && 
                              creator.vendor.creator_title.toLowerCase() === activeCategory.toLowerCase());
        ...
        <section className="mb-10">
          ...
          {isLoading ? (
            ...
            <>
              {filteredVendors.length > 0 ? (
                    ...
                    >
                      <div className="relative h-32 bg-gray-200">
                        <div className="relative w-full h-full">
                          <Image 
                            src={creator.vendor.coverphoto || CoverImage} 
                      ...
                      <div className="p-4">
                        <div className="flex items-center">
                          <div className="relative w-12 h-12 mr-3 overflow-hidden rounded-full">
                            <Image 
                              src={creator.vendor.logo || avatarImage} 
                          ...
                          <div>
                            <div className="flex items-center">
                              <h3 className="font-semibold">{creator.vendor.name}</h3>
                            ...
                            <div className="flex items-center text-xs text-gray-500">
                              <span>{creator.vendor.creator_title || "Creator"}</span>
                              <span className="mx-1">•</span>
                              <span>
                                {typeof followerCounts[creator.vendor.id] !== 'undefined' 
                                  ? `${followerCounts[creator.vendor.id]} follower${followerCounts[creator.vendor.id] !== 1 ? 's' : ''}` 
                                  : 'Followers'}
                              </span>
                            </div>
                          </div>
                        ...
                        <p className="mt-3 mb-3 text-sm text-gray-600 line-clamp-2">{creator.vendor.creator_bio || "No bio available"}</p>
                        
                        <div className="mb-3 text-xs text-gray-600">
                          <span className="font-medium">Popular:</span> {creator.vendor.popular_product || "Creator products"}
                        </div>
                        
                        <button 
                          className="w-full py-2 border border-[#e65100] text-[#e65100] rounded-md text-sm font-medium hover:bg-[#e65100] hover:text-white transition"
                        >
                          Following
                        </button>
                      </div>
                ...
                <div className="p-8 text-center bg-white rounded-lg shadow-sm">
                  ...
                  <h3 className="mb-2 text-xl font-medium">No creators match your filters</h3>
                  <p className="mb-6 text-gray-600">
                    Try adjusting your search terms or category filters to find the creators you're looking for.
                  </p>
                  <button
                    onClick={() => {
                      setActiveCategory('all');
            ...
            <div className="p-8 text-center bg-white rounded-lg shadow-sm">
              ...
              <h3 className="mb-2 text-xl font-medium">Not following any creators yet</h3>
              <p className="mb-6 text-gray-600">
                Follow your favorite creators to see their updates and new releases in one place.
              </p>
              <p className="text-sm text-gray-500">
                Discover creators from the recommendations below and follow them to get started.
              </p>
            </div>
          )}
        </section>
              ...
              {allVendors.map(creator => (
                ...
                >
                  <div className="relative h-32 bg-gray-200">
                    <div className="relative w-full h-full">
                      <Image 
                        src={creator.coverphoto || CoverImage} 
                  ...
                  <div className="p-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <div className="flex items-center text-xs text-gray-500">
                          <span>{creator.creator_title || "Creator"}</span>
                          <span className="mx-1">•</span>
                          <span>
                            {typeof followerCounts[creator.id] !== 'undefined' 
                              ? `${followerCounts[creator.id]} follower${followerCounts[creator.id] !== 1 ? 's' : ''}` 
                              : 'Followers'}
                          </span>
                        </div>
                      </div>
                    ...
                    <p className="mt-3 mb-3 text-sm text-gray-600 line-clamp-2">{creator.creator_bio || "No bio available"}</p>
                    
                    <div className="mb-3 text-xs text-gray-600">
                      <span className="font-medium">Popular:</span> {creator.popular_product || "Creator products"}
                    </div>
