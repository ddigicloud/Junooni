...
export default function CheckoutPageClient({
  ...
  return (
    // FIX 1: overflow-x-hidden on root prevents ANY child from causing horizontal scroll
    <div
      style={{ ...brandStyles, ...(pageBgColor ? { backgroundColor: pageBgColor } : {}) }}
      className={`min-h-screen overflow-x-hidden ${isDark ? "bg-gray-950" : "bg-gray-50"} ${fontClass}`}
    >
      <header
          ...
          <div className="min-w-0 justify-self-start">
            {checkoutSettings.logo_position === "left" ? (
              ...
              >
                ...
                {showBackLink && <span className="hidden sm:inline">Back to store</span>}
              </Link>
            )}
          </div>
      ...
      <div className="max-w-6xl px-4 py-8 mx-auto sm:px-6">
        {/* FIX 2: overflow-hidden here ensures StepIndicator never bleeds outside */}
        <div className="mb-8 overflow-hidden">
          <StepIndicator currentStep={step} brandPrimary={checkoutSettings.accent_color || brandPrimary} />
        </div>

        {/* FIX 3: Order summary moves ABOVE the form on mobile (order-first),
            goes back to right column on lg. This way the 360px panel never
            sits beside the form on narrow screens and can't force the page wider. */}
        <div className="grid lg:grid-cols-[1fr_360px] gap-6 lg:gap-8 items-start">

          {/* ── Order Summary — shown first on mobile, second column on desktop ── */}
          <div
            className="border shadow-sm p-4 sm:p-6 lg:sticky lg:top-24 lg:order-last w-full min-w-0"
            style={{
          ...
          >
            ...
            {activeCart.items?.length === 0 ? (
              <div className="py-6 text-center">
                ...
                <p className={`mt-1 text-xs opacity-60 ${isDark ? "text-white/30" : "text-gray-400"}`}>
              ...
              <div className="mb-4 space-y-4">
                {activeCart.items?.map((item: any) => (
                  <div key={item.id} className="flex items-center gap-3 min-w-0">
                    ...
                    {/* FIX 4: min-w-0 + truncate on title, price never shrinks below content */}
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium truncate ${isDark ? "text-white" : "text-gray-900"}`}>
                        {item.title}
                      </p>
                      {item.variant?.title && item.variant.title !== "Default Title" && (
                        <p className="text-xs text-gray-400 truncate">{item.variant.title}</p>
                      )}
                    </div>
                    <p className={`text-sm font-semibold shrink-0 pl-2 ${isDark ? "text-white" : "text-gray-900"}`}>
          ...
          {/* ── Forms — second on mobile, first column on desktop ── */}
