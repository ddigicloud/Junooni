...
//   // "no-store" instead of "force-cache" — revalidateTag does NOT flush
//   // client-invoked server action fetches, so the cart drawer was always
//   // getting the stale pre-add response. no-store hits Medusa directly.
//   return sdk.client
...
//       billing_address: sameAsBilling ? shippingAddress : shippingAddress,
//     })
...
// }: { cartId: string; shippingMethodId: string }) {
//   const headers = await getAuthHeaders()
