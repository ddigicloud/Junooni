...
import { Search, ShoppingBag, Instagram, Youtube, Twitter, Facebook, ArrowRight, ChevronRight } from "lucide-react"
  ...
  return (
    <div className={`min-h-screen bg-white ${fontClass}`}>
      ...
      {(() => {
        ...
        // Only wrap in sticky if header OR announcement is sticky
...
function MinimalSection({ section, vendor, store, products, categories, collections, brandPrimary }: {
  ...
  switch (section.type) {
      ...
      <section className="relative overflow-hidden bg-gray-50 border-b border-gray-100">
        ...
        {(section.background_image ?? store?.hero_image) && (
          <div className="absolute inset-0">
            <Image src={section.background_image ?? store!.hero_image!} alt="Hero" fill className="object-cover opacity-15" />
          </div>
        )}
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 py-16 md:py-24">
          <div className="flex flex-col md:flex-row items-center gap-12">
            ...
            >
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full text-gray-700 font-semibold border-2 border-gray-200 hover:border-gray-400 transition-all"
                >
                  Browse all
                </Link>
              </div>
            ...
            {(vendor.coverphoto ?? vendor.logo) && (
              <motion.div
                className="shrink-0 w-full md:w-80 lg:w-96"
              ...
              >
                <div className="aspect-square rounded-3xl overflow-hidden shadow-2xl relative">
                  <Image
                    src={vendor.coverphoto ?? vendor.logo!}
                    alt={vendor.name}
                    fill
        ...
        <section id="products" className="py-16 px-4 sm:px-6">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">{section.title ?? "Featured"}</h2>
                <p className="text-sm text-gray-400 mt-1">Hand-picked for you</p>
              </div>
              <Link href={`/${handle}/products`} className="flex items-center gap-1 text-sm font-medium hover:gap-2 transition-all" style={{ color: brandPrimary }}>
    ...
    case "collection": {
      ...
      return (
        <section id="products" className="py-16 px-4 sm:px-6 bg-gray-50">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">{section.title ?? "All Products"}</h2>
    ...
    case "featured_collections": {
      // Show only selected collections, or all collections if none selected
      ...
      return (
        <section className="py-16 px-4 sm:px-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className={`grid ${gridClass} gap-5`}>
              {toShow.map(col => {
                // Find thumbnail: collection thumb → first product in collection → fallback
                ...
                const productCount = products.filter(
                  p => (p as any).collection?.handle === col.handle
                ).length
        ...
        case "divider": {
      // We hijack divider to show collections grid if collections exist
      if (!collections.length) return <hr className="border-gray-100 mx-6" />
      return (
        <section className="py-16 px-4 sm:px-6">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-sm text-gray-400 mt-1">Shop by collection</p>
              </div>
              <Link href={`/${handle}/collections`} className="flex items-center gap-1 text-sm font-medium hover:gap-2 transition-all" style={{ color: brandPrimary }}>
                View all <ChevronRight className="w-4 h-4" />
              </Link>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {collections.slice(0, 4).map(col => {
                // Use collection thumbnail, or first product from product_ids, or fallback
                ...
                return (
                  <Link key={col.id} href={`/${handle}/collections/${col.handle}`}
                    className="group rounded-2xl overflow-hidden border border-gray-100 bg-white shadow-sm hover:shadow-md transition-all">
                    ...
                    <div className="p-4">
                      <p className="font-semibold text-sm text-gray-900">{col.title}</p>
          ...
          <div className={`flex flex-col ${section.image_position === "right" ? "md:flex-row-reverse" : "md:flex-row"} gap-12 items-center`}>
            ...
            {(section.image ?? vendor.coverphoto ?? vendor.logo) && (
              <div className="w-full md:w-2/5 shrink-0">
                <div className="aspect-square rounded-3xl overflow-hidden relative shadow-xl">
                  <Image
                    src={section.image ?? vendor.coverphoto ?? vendor.logo!}
                    alt={vendor.name}
                    fill
                    className="object-cover"
                  />
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-xs uppercase tracking-widest font-semibold" style={{ color: brandPrimary }}>
                  About
                </span>
              </div>
              <h2 className="text-3xl font-bold text-gray-900 mb-4">{section.title ?? vendor.name}</h2>
              <p className="text-gray-600 leading-relaxed text-base">
                {section.text ?? vendor.creator_bio ?? "Official merchandise store."}
              </p>
              {vendor.creator_title && (
                <p className="mt-4 text-sm font-medium" style={{ color: brandPrimary }}>{vendor.creator_title}</p>
              )}
            </div>
          </div>
        </div>
      ...
      return (
        <section className="py-12 px-6 bg-gray-50">
          <div className="max-w-4xl mx-auto text-center">
            {section.title && <h2 className="text-xl font-bold text-gray-900 mb-6">{section.title}</h2>}
    ...
    case "html": {
      ...
      // Wrap in iframe to sandbox styles — prevents CSS leaking into parent page
      ...
      return (
        <section className="py-12 px-4 sm:px-6">
          <div className="max-w-4xl mx-auto">
            {section.title && (
              <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">{section.title}</h2>
            )}
            {/* Video container with YouTube branding play button overlay */}
            <div className="relative w-full rounded-2xl overflow-hidden shadow-xl"
              ...
              <iframe
                ...
                style={{ border: 0 }}
              />
              {/* YouTube branding badge — shows only for YouTube, only when NOT autoplaying */}
      ...
      return (
        <section className="py-12 px-4 sm:px-6">
          <div className="max-w-lg mx-auto">
            {section.title && (
              <h2 className="text-xl font-bold text-gray-900 mb-6 text-center">{section.title}</h2>
            )}
...
function defaultSections(vendor: PublicVendor): StoreSection[] {
  return [
    { type: "hero", headline: vendor.name, subtext: vendor.creator_title ?? undefined, cta_label: "Shop Now" },
    { type: "collection", title: "All Products", limit: 12 },
    { type: "divider" }, // shows collections grid
