...
export default function PageSections({
  ...
  const visible = sections.filter(s => {
    ...
    if (skipTypes.includes(s.type)) return false
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      if (e.data?.type === "STORE_UPDATE") setSelectedSectionId(e.data.selectedId ?? null)
    }
  ...
  return (
    <>
      {visible.map((section, i) => {
        const sId = section.id ?? String(i)
        const isSelected = selectedSectionId === sId
        return (
          <div
            key={sId}
            data-section-id={sId}
            onClick={() => isEditorMode && window.parent?.postMessage({ type: "SECTION_CLICK", sectionId: sId }, "*")}
            onDoubleClick={() => isEditorMode && window.parent?.postMessage({ type: "SECTION_DBLCLICK", sectionId: sId }, "*")}
            className={`relative ${isEditorMode ? "cursor-pointer" : ""}`}
            style={isSelected ? { outline: "2px solid #e65100", outlineOffset: "-2px" } : undefined}
          >
            {isSelected && (
              <div
