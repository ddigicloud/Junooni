...
class MetadataDiscovery {
    ...
    initManyToManyFields(meta, prop) {
        ...
        const fks = props.filter(p => p.reference === enums_1.ReferenceType.MANY_TO_ONE);
        if (pivotMeta) {
    ...
    ensureCorrectFKOrderInPivotEntity(meta, owner) {
        ...
        const fks = Object.values(meta.properties).filter(p => p.reference === enums_1.ReferenceType.MANY_TO_ONE);
        let first, second;
        if (pks.length === 2) {
            [first, second] = pks;
        }
