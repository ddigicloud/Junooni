...
class FileCacheAdapter {
    ...
    async get(name) {
        ...
        const payload = await (0, fs_extra_1.readJSON)(path);
    ...
    async set(name, data, origin) {
        ...
        await (0, fs_extra_1.writeJSON)(path, { data, origin, hash, version: this.VERSION }, opts);
    }
    /**
     * @inheritDoc
     */
    ...
    async clear() {
        ...
        await Promise.all(files.map(file => (0, fs_extra_1.unlink)(file)));
    }
    async path(name) {
        await (0, fs_extra_1.ensureDir)(this.options.cacheDir);
        return `${this.options.cacheDir}/${name}.json`;
    }
    async getHash(origin) {
        origin = Utils_1.Utils.absolutePath(origin, this.baseDir);
        if (!await (0, fs_extra_1.pathExists)(origin)) {
            return null;
        }
        const contents = await (0, fs_extra_1.readFile)(origin);
