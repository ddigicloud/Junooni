"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _UDHK7OEScjs = require('../__chunks/UDHK7OES.cjs');


var _JLUMKCNRcjs = require('../__chunks/JLUMKCNR.cjs');
require('../__chunks/HLFPESUH.cjs');
require('../__chunks/SQHULPZ2.cjs');
require('../__chunks/RVRXLUKN.cjs');
require('../__chunks/4EKRCHT6.cjs');
require('../__chunks/CWQU4R75.cjs');
require('../__chunks/FVVTEJ4K.cjs');
require('../__chunks/KPH4LMZO.cjs');
require('../__chunks/ZVS6SBRR.cjs');
require('../__chunks/VF7H2WBX.cjs');
require('../__chunks/EJVJIL4S.cjs');
require('../__chunks/6TI3NXX6.cjs');
require('../__chunks/PV4FS33B.cjs');
require('../__chunks/PTLTNFQI.cjs');
require('../__chunks/T272CHVS.cjs');
require('../__chunks/ZRZUZ7E7.cjs');
require('../__chunks/76LCMKUI.cjs');
require('../__chunks/L5A2ID7S.cjs');
require('../__chunks/FDRJDQ5Y.cjs');
require('../__chunks/7EQBAZ46.cjs');

// src/menu/menu-provider.tsx
var _jsxruntime = require('react/jsx-runtime');
function MenuProvider(props = {}) {
  const store = _UDHK7OEScjs.useMenuStore.call(void 0, props);
  return /* @__PURE__ */ _jsxruntime.jsx.call(void 0, _JLUMKCNRcjs.MenuContextProvider, { value: store, children: props.children });
}


exports.MenuProvider = MenuProvider;
