"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _4KJXPBQLcjs = require('../__chunks/4KJXPBQL.cjs');


var _JLUMKCNRcjs = require('../__chunks/JLUMKCNR.cjs');
require('../__chunks/HLFPESUH.cjs');
require('../__chunks/6BDNEU4N.cjs');
require('../__chunks/EPZ3LJJN.cjs');
require('../__chunks/4KO75GA3.cjs');
require('../__chunks/FME6FBOS.cjs');
require('../__chunks/CWQU4R75.cjs');
require('../__chunks/FVVTEJ4K.cjs');
require('../__chunks/KPH4LMZO.cjs');
require('../__chunks/ZVS6SBRR.cjs');
require('../__chunks/VF7H2WBX.cjs');
require('../__chunks/EJVJIL4S.cjs');
require('../__chunks/KYD66ZXY.cjs');
require('../__chunks/T3QB4FR3.cjs');




var _6TI3NXX6cjs = require('../__chunks/6TI3NXX6.cjs');
require('../__chunks/L5A2ID7S.cjs');
require('../__chunks/FDRJDQ5Y.cjs');



var _7EQBAZ46cjs = require('../__chunks/7EQBAZ46.cjs');

// src/menu/menu-dismiss.tsx
var TagName = "button";
var useMenuDismiss = _6TI3NXX6cjs.createHook.call(void 0, 
  function useMenuDismiss2(_a) {
    var _b = _a, { store } = _b, props = _7EQBAZ46cjs.__objRest.call(void 0, _b, ["store"]);
    const context = _JLUMKCNRcjs.useMenuScopedContext.call(void 0, );
    store = store || context;
    props = _4KJXPBQLcjs.useHovercardDismiss.call(void 0, _7EQBAZ46cjs.__spreadValues.call(void 0, { store }, props));
    return props;
  }
);
var MenuDismiss = _6TI3NXX6cjs.forwardRef.call(void 0, function MenuDismiss2(props) {
  const htmlProps = useMenuDismiss(props);
  return _6TI3NXX6cjs.createElement.call(void 0, TagName, htmlProps);
});



exports.MenuDismiss = MenuDismiss; exports.useMenuDismiss = useMenuDismiss;
