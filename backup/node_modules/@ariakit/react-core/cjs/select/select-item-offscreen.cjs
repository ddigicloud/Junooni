"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _BB77FAWKcjs = require('../__chunks/BB77FAWK.cjs');
require('../__chunks/2C4LCZ5B.cjs');


var _BMW3SZHTcjs = require('../__chunks/BMW3SZHT.cjs');
require('../__chunks/KUPB4YJ5.cjs');


var _QZEXWZZZcjs = require('../__chunks/QZEXWZZZ.cjs');
require('../__chunks/MNH5LHM4.cjs');
require('../__chunks/DQ3AW7HU.cjs');
require('../__chunks/FME6FBOS.cjs');
require('../__chunks/MIBORXQW.cjs');
require('../__chunks/CWQU4R75.cjs');
require('../__chunks/FVVTEJ4K.cjs');


var _3YJARQUNcjs = require('../__chunks/3YJARQUN.cjs');
require('../__chunks/ZVS6SBRR.cjs');
require('../__chunks/VF7H2WBX.cjs');
require('../__chunks/EJVJIL4S.cjs');
require('../__chunks/KYD66ZXY.cjs');
require('../__chunks/T3QB4FR3.cjs');


var _6TI3NXX6cjs = require('../__chunks/6TI3NXX6.cjs');
require('../__chunks/76LCMKUI.cjs');


var _L5A2ID7Scjs = require('../__chunks/L5A2ID7S.cjs');
require('../__chunks/FDRJDQ5Y.cjs');




var _7EQBAZ46cjs = require('../__chunks/7EQBAZ46.cjs');

// src/select/select-item-offscreen.tsx
var _jsxruntime = require('react/jsx-runtime');
var TagName = "div";
function useSelectItemOffscreen(_a) {
  var _b = _a, { store, value } = _b, props = _7EQBAZ46cjs.__objRest.call(void 0, _b, ["store", "value"]);
  const context = _QZEXWZZZcjs.useSelectScopedContext.call(void 0, );
  store = store || context;
  return _BB77FAWKcjs.useCompositeItemOffscreen.call(void 0, _7EQBAZ46cjs.__spreadValues.call(void 0, { store, value }, props));
}
var SelectItem2 = _6TI3NXX6cjs.forwardRef.call(void 0, function SelectItem3(_a) {
  var _b = _a, {
    offscreenBehavior,
    offscreenRoot
  } = _b, props = _7EQBAZ46cjs.__objRest.call(void 0, _b, [
    "offscreenBehavior",
    "offscreenRoot"
  ]);
  const _a2 = useSelectItemOffscreen(_7EQBAZ46cjs.__spreadValues.call(void 0, {
    offscreenBehavior,
    offscreenRoot
  }, props)), { active, ref } = _a2, rest = _7EQBAZ46cjs.__objRest.call(void 0, _a2, ["active", "ref"]);
  const allProps = _7EQBAZ46cjs.__spreadProps.call(void 0, _7EQBAZ46cjs.__spreadValues.call(void 0, _7EQBAZ46cjs.__spreadValues.call(void 0, {}, rest), props), { ref: _L5A2ID7Scjs.useMergeRefs.call(void 0, ref, props.ref) });
  if (active) {
    return /* @__PURE__ */ _jsxruntime.jsx.call(void 0, _BMW3SZHTcjs.SelectItem, _7EQBAZ46cjs.__spreadValues.call(void 0, {}, allProps));
  }
  const _b2 = allProps, {
    store,
    value,
    getItem,
    hideOnClick,
    setValueOnClick,
    preventScrollOnKeyDown,
    focusOnHover,
    blurOnHoverEnd
  } = _b2, htmlProps = _7EQBAZ46cjs.__objRest.call(void 0, _b2, [
    "store",
    "value",
    "getItem",
    "hideOnClick",
    "setValueOnClick",
    "preventScrollOnKeyDown",
    "focusOnHover",
    "blurOnHoverEnd"
  ]);
  const Component = _3YJARQUNcjs.Role[TagName];
  return /* @__PURE__ */ _jsxruntime.jsx.call(void 0, Component, _7EQBAZ46cjs.__spreadValues.call(void 0, {}, htmlProps));
});



exports.SelectItem = SelectItem2; exports.useSelectItemOffscreen = useSelectItemOffscreen;
