...
// Add this useEffect near the top of your component, right after all the state declarations
useEffect(() => {
  ...
  if (!token) {
    setError("Authentication required. Please log in to create or edit products.");
    
    // Redirect to sign-in page using navigate
    navigate({ to: '/sign-in' });
    return;
  }
}, [navigate]); // Add navigate to the dependency array

// Add this useEffect right after the authentication check in ProductForm.jsx
useEffect(() => {
  const verifyGST = async () => {
    try {
      // Check authentication first (redundant with your existing check, but keeping for completeness)
      const token = localStorage.getItem('vendorToken');
      if (!token) return;
      ...
      const gstStatus = data.vendor?.gst_verification_status || "pending";
      
      // If GST is not verified, redirect to onboarding/business-details
      if (gstStatus !== "verified") {
        setError("GST verification required to create products.");
        
        // Use window.location for a full page redirect to ensure the modal shows up properly
        window.location.href = '/onboarding?step=business-details';
        return;
      }
    } catch (error) {
      console.error('Error verifying GST status:', error);
      setError("Unable to verify GST status. Please try again.");
    }
  };
  
  // Only run GST verification if we're already authenticated
  const token = localStorage.getItem('vendorToken');
  if (token) {
    verifyGST();
  }
