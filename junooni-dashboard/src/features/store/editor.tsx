...
// function getStoreUrl(handle: string, customDomain?: string | null): string {
//   if (customDomain) return `https://${customDomain}`
//   if (import.meta.env.PROD) return `https://${handle}.${ROOT_DOMAIN}`
//   return `http://localhost:${STORE_PORT}/${handle}`
// }

// function getPreviewUrl(handle: string): string {
...
//   const selectedSection = selectedId ? sections.find(s => s.id === selectedId) ?? null : null
//   const previewUrl = vendorHandle ? getPreviewUrl(vendorHandle) : null
//   const isLive = store.status === "live"
...
//             className={`flex-1 rounded-r-lg px-2 py-1.5 text-xs font-mono focus:outline-none focus:border-orange-500 ${isDark ? "bg-gray-800 border border-gray-700 text-gray-200" : "bg-white border border-gray-300 text-gray-800"}`} />
//         </div>
//         <a href={`${getStoreUrl(vendorHandle)}/p/${draft.slug}`} target="_blank" rel="noopener noreferrer"
//           className="text-[10px] text-orange-400 hover:text-orange-300 mt-1 flex items-center gap-1">
//           <ExternalLink className="w-2.5 h-2.5" />Preview page
//         </a>
//       </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <a href={getPageUrl(vendorHandle, draft.slug, store?.custom_domain)} target="_blank" rel="noopener noreferrer"
          className="text-[10px] text-orange-400 hover:text-orange-300 mt-1 flex items-center gap-1">
          <ExternalLink className="w-2.5 h-2.5" />Preview page
        </a>
      </div>
