...
// Interface for onboarding completion status — only basic-info and creator-profile
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  const formatStatus = (status: string) => {
    switch (status.toLowerCase()) {
      case "not_fulfilled": return "Pending";
      ...
      default: return status.split('_').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(' ');
    }
  };
...
const checkVendorExists = (vendorData: any) => {
  ...
  if (vendorData.vendor) return !!vendorData.vendor.id;
  if (vendorData.id) return true;
  return Object.keys(vendorData).length > 0 && !isEmptyResponse(vendorData);
};

// ── checkOnboardingCompletion: only basic-info and creator-profile ──
const checkOnboardingCompletion = (vendorData: any): OnboardingStatus => {
  console.log('vendor.id from API:', vendorData.vendor?.id)

  if (!vendorData || !vendorData.vendor) {
    return {
      isComplete: false,
      completedSteps: [],
      missingSteps: ['basic-info', 'creator-profile'],
      completionPercentage: 0,
      stepDetails: {
        'basic-info': {
          isComplete: false,
          missingFields: ['Store Name', 'Handle', 'First Name', 'Last Name'],
          requiredFields: ['Store Name', 'Handle', 'First Name', 'Last Name']
        },
        'creator-profile': {
          isComplete: false,
  ...
  if (hasBasicInfo) completedSteps.push('basic-info');
  else missingSteps.push('basic-info');

  // ── Creator Profile (recommended) ──
  ...
  if (hasCreatorProfile) completedSteps.push('creator-profile');
  else missingSteps.push('creator-profile');

  // 2 meaningful steps: basic-info + creator-profile
  const totalSteps = 2;
  const completionPercentage = Math.round((completedSteps.length / totalSteps) * 100);

  // Onboarding is complete when basic info is done (creator profile is recommended, not blocking)
...
const OnboardingProgressBanner = ({ onboardingStatus, vendorName }: { 
  ...
  // basic-info is Required; creator-profile is Recommended
  const getStepPriority = (step: string) => 
    step === 'basic-info' ? 'Required' : 'Recommended';

  const getStepPriorityColor = (step: string) => 
    step === 'basic-info' ? 'text-red-600' : 'text-amber-600';

  // Dismissed state — slim persistent bar
  if (isDismissed) {
    return (
      <div 
...
const DashboardPage = () => {
  ...
  const transformVendorOrders = (orderData: any) => {
    ...
    return ordersArray.map((order: any, index: number) => {
      try {
        ...
        if (custom_display_id === 0) custom_display_id = index + 1;
        ...
        const vendor_items: VendorOrderItem[] = (order.vendor_items || []).map((item: any, itemIndex: number) => {
          ...
          let product_cost = 0;
          if (item.product_cost !== undefined) product_cost = safeNumber(item.product_cost);
          else if (item.merged_metadata?.product_cost !== undefined) product_cost = safeNumber(item.merged_metadata.product_cost);
        ...
        return {
          ...
          vendor_total: 0, vendor_subtotal: 0, vendor_shipping_total: 0, vendor_tax_total: 0,
          vendor_items: [],
          ...
          claims: [], returns: [], claim_items: [], return_items: [],
    ...
    try {
      ...
      try {
        ...
        if (!orderResponse.ok) {
          ...
          let ordersArray = [];
          if (orderData.orders && Array.isArray(orderData.orders)) ordersArray = orderData.orders;
          else if (orderData.data && Array.isArray(orderData.data)) ordersArray = orderData.data;
          else if (Array.isArray(orderData)) ordersArray = orderData;
      ...
      try {
        ...
        if (!productResponse.ok) {
          ...
          const transformedProducts = productsArray.slice(0, 3).map((product: any): Product => ({
            ...
            price: typeof product.price === 'number' ? product.price : product.price?.value ? parseFloat(product.price.value) : 0,
  ...
  const calculateStats = () => {
    if (!orders || !Array.isArray(orders) || orders.length === 0) {
      return { totalRevenue: 0, pendingOrders: 0, completedOrders: 0, totalOrders: 0, todayRevenue: 0, todayOrders: 0, processingOrders: 0, shippedOrders: 0 };
    }
    
    const validOrders = orders.filter(order => order && typeof order === 'object' && typeof order.vendor_total === 'number' && order.fulfillment_status);
    const totalRevenue = validOrders.reduce((sum, order) => sum + (order.vendor_total || 0), 0);
    const pendingOrders = validOrders.filter(o => o.fulfillment_status === "not_fulfilled" || o.fulfillment_status === "pending").length;
    const completedOrders = validOrders.filter(o => ["fulfilled","delivered","completed"].includes(o.fulfillment_status)).length;
    const processingOrders = validOrders.filter(o => ["processing","partially_fulfilled"].includes(o.fulfillment_status)).length;
    const shippedOrders = validOrders.filter(o => ["shipped","partially_shipped"].includes(o.fulfillment_status)).length;
    
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const tomorrow = new Date(today); tomorrow.setDate(tomorrow.getDate() + 1);
    const todayOrders = validOrders.filter(o => { const d = new Date(o.created_at); return d >= today && d < tomorrow; });
    const todayRevenue = todayOrders.reduce((sum, o) => sum + (o.vendor_total || 0), 0);
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center text-base sm:text-lg">
                  <Package className="w-4 h-4 mr-2 text-gray-500 sm:w-5 sm:h-5" />
                  Recent Orders
                </CardTitle>
                        ...
                        <div className="p-3 transition-all border border-gray-100 rounded-lg cursor-pointer sm:p-4 bg-white hover:shadow-md hover:border-[#e65100]">
                          ...
                          <div className="sm:hidden">
                            ...
                            <div className="flex flex-wrap gap-1.5 mb-2.5">
                              ...
                              {order.has_returns && (
                                <Badge variant="outline" className="text-xs text-orange-600 border-orange-200 bg-orange-50">Return</Badge>
                              )}
                              {order.has_claims && (
                                <Badge variant="outline" className="text-xs text-blue-600 border-blue-200 bg-blue-50">Claim</Badge>
                              )}
                            </div>
                            <div className="flex items-center justify-between pt-2.5 border-t border-gray-100">
                              <span className="text-xs font-medium text-gray-500">Total Amount</span>
                              <span className="text-base font-bold" style={{ color: BRAND.primary }}>
                                ...
                                <div className="flex items-center gap-3 sm:text-right">
                                  <div>
                                    ...
                                    <p className="flex items-center gap-1 text-xs text-gray-500">
                                      ...
                                      <span>item{order.vendor_items.reduce((acc, item) => acc + item.quantity, 0) !== 1 ? 's' : ''}</span>
                                    </p>
                                  </div>
                                </div>
                              </div>
          ...
          <div className="space-y-6">
            <Card className="shadow-md">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="flex items-center text-base sm:text-lg">
                    <ShoppingBag className="w-4 h-4 mr-2 text-gray-500 sm:w-5 sm:h-5" />
                    Your Products
                  </CardTitle>
                  <Button variant="ghost" size="sm" className="h-8 text-xs sm:text-sm" asChild>
