          ...
          >
            ...
            {marketplaceStatus === "rejected" && (
              <div className="border-t border-red-100 pt-3 space-y-2">
                ...
                <span
                  onClick={(e) => { e.stopPropagation(); handleApplyToMarketplace() }}
                  className="inline-flex items-center gap-1 text-xs font-semibold underline text-red-700"
                >
...
export function StoreModeBadge({
  ...
  const showBadge = !hasNothingSetUp || marketplaceStatus === "pending" || marketplaceStatus === "rejected"
  if (!showBadge) return null

  const both = pref.sell_on_marketplace && pref.sell_on_own_store
  let label = both ? "Both stores" : pref.sell_on_marketplace ? "Marketplace" : "Own store"
