...
export const renderMockupDirectly = async (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      // ─── QUALITY FIX #1: Supersample at 2× then downsample ──────────────
      // Rendering at 2× then downsampling to targetResolution gives
      // significantly sharper edges than rendering directly at targetResolution.
      // This is especially important for design artwork with fine details.
      const SUPERSAMPLE = 2;
      ...
      // ─── QUALITY FIX #2: Force highest quality settings on every context ─
      ctx.imageSmoothingEnabled = true;
      ctx.imageSmoothingQuality = 'high';
      ...
      for (const mockupArea of mockup.area || []) {
        ...
        // All coordinates now in renderRes space (supersampled)
        ...
        // ─── QUALITY FIX #3: Design canvas at NATIVE source resolution ─────
        // Previously the design canvas was sized to mockupAreaWidth (final pixel
        // count). Now we render at native source resolution and only scale down
        // when compositing — preserving every pixel of the uploaded artwork.
        //
        // Strategy:
        //   a) Find the largest source image in this area to set a "native" res
        //   b) Render all elements at that native resolution
        //   c) Scale the finished design canvas down onto the mockup canvas
        //      using drawImage (which triggers high-quality bicubic downscale)

        // 2a: Determine native design canvas size from actual image sources
        ...
        for (const element of visibleElements) {
          ...
          // Scale that keeps source pixels 1:1 on the design canvas
          // (printable-area → mockup-area mapping already baked in uniformScale)
          const nW = Math.round(src.naturalWidth  * (renderedWidth  / element.width)  * uniformScale * (src.naturalWidth  / element.width));
          const nH = Math.round(src.naturalHeight * (renderedHeight / element.height) * uniformScale * (src.naturalHeight / element.height));

          // Use the maximum needed resolution but cap at 8192 to avoid OOM
          nativeWidth  = Math.min(8192, Math.max(nativeWidth,  nW));
          nativeHeight = Math.min(8192, Math.max(nativeHeight, nH));
        }

        // 2b: Render design elements onto isolated transparent canvas at native res
        const designCanvas   = document.createElement('canvas');
        ...
        // Mapping from printable-area coords → native design canvas coords
        ...
        for (const element of visibleElements) {
          ...
          // ─── QUALITY FIX #4: Draw image at its own natural resolution ────
          // Instead of drawing at a pre-scaled size, draw the image at a size
          // that maps its natural pixels 1:1 where possible, letting the browser
          // bicubic-downsample only as a final step rather than upsampling small
          // images to fill a large canvas (which causes blur).
          const src = element.image as HTMLImageElement;
          if (src.naturalWidth && src.naturalWidth > 0) {
            // Draw at natural-resolution scale relative to our native canvas
            const nativeElemW = elemWidthInDesign;
            const nativeElemH = elemHeightInDesign;
            dCtx.drawImage(
              element.image,
              -nativeElemW / 2, -nativeElemH / 2,
              nativeElemW, nativeElemH
            );
          } else {
            dCtx.drawImage(
              element.image,
        ...
        // 2c: Apply area transforms + alpha masks on a full-renderRes canvas
        const maskedDesignCanvas   = document.createElement('canvas');
        ...
        if (mCtx) {
          ...
          if (hasTransform) {
            ...
            mCtx.translate(-areaCentreX, -areaCentreY);
            // ─── QUALITY FIX #5: Scale nativeDesignCanvas → renderRes here ─
            // This is the single bicubic downsample step. The browser performs
            // this much better than repeated intermediate draws.
            mCtx.drawImage(designCanvas, mockupAreaX, mockupAreaY, mockupAreaWidth, effectiveMockupHeight);
            mCtx.restore();
          } else {
            mCtx.drawImage(designCanvas, mockupAreaX, mockupAreaY, mockupAreaWidth, effectiveMockupHeight);
      ...
      // ─── QUALITY FIX #6: Downsample supersampled canvas → targetResolution ─
      // This is the final step: downsample 2× → 1× with bicubic filtering.
      // This eliminates aliasing and produces a sharper result than rendering
      // directly at targetResolution would.
