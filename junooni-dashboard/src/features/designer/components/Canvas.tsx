...
const DynamicTShirtDesigner: React.FC<TShirtDesignerProps> = ({ productData }) => {
  // Add navigate hook from Tanstack Router
  ...
  // New function to handle "Add to Store" action
  const handleAddToStore = () => {
    // Collect all relevant product data
    const designData = {
      // Basic product info
      productInfo: {
        title: `Custom ${productData.name}`,
        description: `Customized ${productData.name} with unique design`,
        sku: productData.sku,
      ...
      options: [
        {
          title: "Color",
          optionValues: selectedColors.map(color => color.name)
        },
        {
          title: "Size",
          optionValues: selectedSizes
        }
      ],
      
      // Design data for potential thumbnails or previews
      designElements: {
        front: designElements.front,
        back: designElements.back,
        left: designElements.left,
        right: designElements.right
      },
      
      // Selected colors with hex values for proper display
      colorDetails: selectedColors.map(color => ({
        name: color.name,
        value: color.value
      })),
      
      // Technology used (for additional product details)
      printingTechnology: activeTechnology,
      
      // Default price (could be calculated based on complexity if needed)
      price: productData.cost || 24.99
    };

    // Convert design elements to image URLs if possible
    // We'll need to generate images from the design for product thumbnails
    const designImages = generateDesignThumbnails();
    
    // Navigate to the product creation page with the design data
    navigate({ 
      to: '/designer/create',
      search: {
        fromDesigner: 'true',
      },
      // Pass the data as state to the route
      state: { designData, designImages }
    });
  };

  // Helper function to generate thumbnails from designs
  const generateDesignThumbnails = () => {
    // If we have a stage reference, we can export the canvas as an image
    if (stageRef.current) {
      const dataURLs = {};
      
      // For the active side only (for simplicity)
      // In a complete implementation, we would generate images for all sides
      const dataURL = stageRef.current.toDataURL();
      dataURLs[activeSide] = dataURL;
  ...
  // Sidebar content based on active tab and view
  const renderSidebarContent = () => {
    // Special sidebar content for Preview view - Modified for Add to Store functionality
    if (activeView === 'preview') {
      return (
        <div className="p-4">
          <h3 className="mb-4 text-lg font-medium">Product Configuration</h3>
          ...
          {/* Add to Store Button - Now calls our new function */}
          <button 
            className="w-full py-3 mb-3 font-medium text-white bg-green-600 rounded-md hover:bg-green-700"
