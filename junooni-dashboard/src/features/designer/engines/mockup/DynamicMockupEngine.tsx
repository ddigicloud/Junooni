    ...
    try {
      ...
      for (let index = 0; index < area.length; index++) {
        ...
        try {
          ...
          // Apply clipping mask
          // Apply clipping mask - PROPERLY for transparency
          const clipMask = new PIXI.Graphics();
          clipMask.beginFill(0xffffff, 1); // Alpha 1 for proper masking
          clipMask.drawRect(0, 0, uvAreaWidth, uvAreaHeight);
          clipMask.endFill();

          // CRITICAL FIX: Don't add mask as child - it shouldn't render
          // Just assign it as the mask property
          designContainer.mask = clipMask;

          // Add mask to a separate container that won't be visible
          const maskContainer = new PIXI.Container();
          maskContainer.addChild(clipMask);
          app.stage.addChild(maskContainer);
          maskContainer.visible = false; // Make container invisible

          // Position mask to match design container in world space
          clipMask.x = designContainer.x;
          clipMask.y = designContainer.y;
          clipMask.scale.set(designContainer.scale.x, designContainer.scale.y);
          clipMask.rotation = designContainer.rotation;

          debugMessage(`Clipping mask applied (invisible) at size ${uvAreaWidth.toFixed(1)}x${uvAreaHeight.toFixed(1)}`);
