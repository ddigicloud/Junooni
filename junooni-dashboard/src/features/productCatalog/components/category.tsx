...
// ─── Cache helpers ────────────────────────────────────────────────────────────
// TTL cache used only for category list / slug fetches (nav data)
...
function cacheGet<T>(key: string): T | null {
  try {
    ...
    // Support both TTL-wrapped { data, ts } and plain objects
...
// ⚠️ Plain write — no TTL wrapper — ProductPage reads product_N with plain JSON.parse
// Wrapping in { data, ts } would make product.cost undefined → ₹NaN
...
async function fetchCategoryList(): Promise<Category[]> {
  ...
  cacheSet(cacheKey, data.docs);
  return data.docs;
}

async function fetchCategoryBySlug(slug: string): Promise<Category | null> {
  const cacheKey = `cat_slug_${slug}_v2`;
  const cached = cacheGet<Category>(cacheKey);
  if (cached) return cached;
  const res = await fetch(
    `${vite_payload}/api/categories?where[slug][equals]=${encodeURIComponent(slug)}&limit=1&depth=2`,
    { credentials: 'include' }
  );
...
const CategoryPage: React.FC = () => {
  ...
  const targetSlug = useMemo(() => {
    ...
    const slugParts = parts.slice(idx + 1).filter(Boolean);
  ...
  useEffect(() => {
    ...
    // Nav loads independently and fast (shallow depth=1)
    fetchCategoryList()
      .then(docs => { if (mounted) { setCategories(docs); setLoadingNav(false); } })
      .catch(() => { if (mounted) setLoadingNav(false); });

    // Products scoped to just this slug (depth=2 on one category only)
    fetchCategoryBySlug(targetSlug)
      .then(cat => {
        if (!mounted) return;
        setCurrentCategory(cat);
        if (cat?.products?.length) {
          const sorted = [...cat.products].sort((a, b) => b.id - a.id);
          setApiProducts(sorted);
          setSimplifiedProducts(sorted.map(toSimplified));
        } else {
          setApiProducts([]);
          setSimplifiedProducts([]);
        }
      ...
      .catch(() => { if (mounted) setLoadingProducts(false); });

    return () => { mounted = false; };
  }, [targetSlug]);

  const { availableColors, availableSizes, availableTechnologies } = useMemo(() => {
    const colors: ColorOption[] = [], sizes: SizeOption[] = [], technologies: PrintingTechnology[] = [];
    apiProducts.forEach(p => {
      p.colorOptions?.forEach(c => { if (!colors.some(x => x.id === c.id)) colors.push(c); });
      p.sizeOptions?.forEach(s => { if (!sizes.some(x => x.id === s.id)) sizes.push(s); });
      p.printingTechnologies?.forEach(t => { if (!technologies.some(x => x.id === t.id)) technologies.push(t); });
    });
    return { availableColors: colors, availableSizes: sizes, availableTechnologies: technologies };
  }, [apiProducts]);
  ...
  const getProductCardData = useCallback((productId: string): Product | null => {
    const p = apiProducts.find(x => x.id === parseInt(productId));
    if (!p) return null;
    return {
  ...
  // Plain write — no TTL wrapper — so ProductPage's JSON.parse gets the product directly
  const storeProduct = useCallback((productId: string) => {
    const p = apiProducts.find(x => x.id === parseInt(productId));
    if (p) storeProductPlain(p.id, p);
  }, [apiProducts]);
              ...
              {loadingProducts ? (
                ...
                <>
                  ...
                  {filteredProducts.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-6 product-grid sm:grid-cols-2 lg:grid-cols-3">
                      {filteredProducts.map((product, index) => {
                        const cardData = getProductCardData(product.id);
