...
// ─── Module-level cache ───────────────────────────────────────────────────────
// Stores the already-PARSED Category tree, not raw API data.
// Zero re-parse overhead. Persists the entire tab session.

const CACHE_TTL = 15 * 60 * 1000 // 15 minutes

let _parsed: { data: Category[]; ts: number } | null = null
let _inFlight: Promise<Category[]> | null = null // dedup concurrent mounts

function cacheGet(): Category[] | null {
  if (!_parsed) return null
  if (Date.now() - _parsed.ts > CACHE_TTL) { _parsed = null; return null }
  return _parsed.data
}

function cacheSet(data: Category[]): void {
  _parsed = { data, ts: Date.now() }
}
...
// ─── Select fields — only request what parseHeaderNavItems needs ──────────────
// Cuts Payload response size dramatically vs depth=2 with no selects.
// navItems → link.reference.value needs: id, title, slug only.

const SELECT_FIELDS = [
  "select[navItems][link][type]=true",
  "select[navItems][link][reference]=true",
  "select[navItems][link][label]=true",
  "select[navItems][children][link][type]=true",
  "select[navItems][children][link][reference]=true",
  "select[navItems][children][link][label]=true",
  "select[navItems][children][subChildren][link][type]=true",
  "select[navItems][children][subChildren][link][reference]=true",
  "select[navItems][children][subChildren][link][label]=true",
].join("&")
...
// ─── Single fetch with in-flight dedup ───────────────────────────────────────
// depth=1 instead of depth=2: Payload only resolves one level of relations.
// The navItems structure already embeds child/subChild references inline,
// so depth=1 is sufficient — depth=2 was fetching grandchild fields we discard.
...
async function fetchNavCategories(): Promise<Category[]> {
  ...
  _inFlight = (async () => {
    try {
      const url = `${vite_payload}/api/globals/header?depth=1&${SELECT_FIELDS}`
      const res = await fetch(url, {
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        // Cache at HTTP level for 60s — browser won't even hit the network
        // on repeated navigations within the same minute
        cache: 'default',
      })
      if (!res.ok) return []
      ...
      const categories = parseHeaderNavItems(header.navItems)
      cacheSet(categories) // store parsed tree, not raw JSON
      return categories
    } catch (err) {
      console.error('[Navbar] fetch error:', err)
      return []
...
// ─── Kick off fetch immediately at module load ────────────────────────────────
// This runs the moment the JS bundle is parsed — before any component mounts.
// By the time Navbar renders, the fetch is already in-flight (or done).
fetchNavCategories()
...
const Navbar = () => {
  // Initialize directly from cache — zero flicker on page 2+ / cached navigations
  const [organizedCategories, setOrganizedCategories] = useState<Category[]>(
    () => cacheGet() ?? []
  )
  // Only show loading skeleton if we have no data at all
  const [loading, setLoading] = useState(() => cacheGet() === null)
  const [activeCategory, setActiveCategory] = useState<number | null>(null)
  const [mobileOpen, setMobileOpen] = useState(false)

  useEffect(() => {
    // Cache already populated by useState initializer or module-level prefetch
    if (cacheGet()) {
      setOrganizedCategories(cacheGet()!)
      setLoading(false)
      return
    }

    // Not cached yet — fetchNavCategories() is likely already in-flight from
    // the module-level call above, so this just awaits the same promise
    fetchNavCategories().then((categories) => {
      setOrganizedCategories(categories)
      setLoading(false)
    })
