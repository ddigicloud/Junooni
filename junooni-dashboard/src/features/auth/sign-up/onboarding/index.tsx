...
const handleSaveAndExit = async () => {
  try {
    ...
    // REPLACE WITH:
    const firstName = vendorData?.vendor?.admins?.[0]?.first_name?.trim();
    const lastName = vendorData?.vendor?.admins?.[0]?.last_name?.trim();
    const handle = vendorData?.vendor?.handle?.trim();
    const storeName = vendorData?.vendor?.name?.trim();

    const missing: string[] = [];
    if (!firstName) missing.push("First Name");
    if (!lastName) missing.push("Last Name");
    if (!handle) missing.push("Handle");
    if (!storeName) missing.push("Store Name");
    ...
    if (missing.length > 0) {
      ...
      const isBasicInfoMissing = !firstName || !lastName || !handle;
      setCurrentStep(isBasicInfoMissing ? "basic-info" : "creator-profile");
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }
