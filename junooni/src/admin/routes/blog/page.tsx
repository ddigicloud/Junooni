...
function CategoryManager({
  categories, loading, onAdd, onDelete, onClose,
}: {
  ...
  const handleAdd = async () => {
    ...
    if (!trimmed) { toast.error("Please enter a category name"); return }
    setSaving(true)
    try { await onAdd(trimmed); setNewLabel("") } finally { setSaving(false) }
  }
  ...
  return (
    <div onClick={(e) => { if (e.target === e.currentTarget) onClose() }}
      ...
      <div style={{ width: "100%", maxWidth: 460, background: "#ffffff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 24, display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 24px 64px rgba(0,0,0,0.18)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <Heading level="h2">Manage Categories</Heading>
            <Text size="small" className="text-ui-fg-subtle">
              {loading ? "Loading..." : `${categories.length} categories · saved to database`}
            </Text>
          </div>
          <button type="button" onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#9ca3af", padding: 4, borderRadius: 4, display: "flex", alignItems: "center" }}>
            <XMark />
          </button>
        </div>
        <div style={{ display: "flex", gap: 8, padding: "10px 12px", background: "#f3f4f6", borderRadius: 8, border: "1px solid #e5e7eb" }}>
          <input ref={inputRef} value={newLabel} onChange={(e) => setNewLabel(e.target.value)}
            ...
            placeholder="e.g. Creator Spotlight"
            style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: "#111827", fontSize: 14, padding: "2px 0" }} />
          <button type="button" onClick={handleAdd} disabled={saving}
            style={{ padding: "6px 16px", background: saving ? "#6b7280" : "#111827", color: "#fff", border: "none", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}>
            {saving ? "Adding..." : "+ Add"}
          </button>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 280, overflowY: "auto" }}>
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              <Text size="small" className="text-ui-fg-subtle">No categories yet</Text>
            </div>
          ) : categories.map((cat) => (
            <div key={cat.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 8 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <Text size="small" weight="plus">{cat.label}</Text>
                <span style={{ fontFamily: "monospace", fontSize: 11, color: "#6b7280" }}>{cat.value}</span>
              </div>
              <button type="button" onClick={() => onDelete(cat.id)} title="Delete"
                style={{ background: "none", border: "none", cursor: "pointer", color: "#f87171", padding: 4, borderRadius: 4, display: "flex", alignItems: "center" }}>
                <Trash />
              </button>
            </div>
          ))}
...
function hasCustomMarkup(html: string): boolean {
  return /<style[\s\S]*?>/i.test(html) || /<meta\s/i.test(html) || /<link\s/i.test(html) || /<head[\s\S]*?>/i.test(html)
}
...
function RichTextEditor({ value, onChange }: { value: string; onChange: (html: string) => void }) {
  ...
  const [sourceValue, setSourceValue] = useState(value)
  const [isCustomHtml, setIsCustomHtml] = useState(() => hasCustomMarkup(value))
  const [mode, setMode] = useState<EditorMode>(() => hasCustomMarkup(value) ? "html" : "visual")

  const handleModeChange = (newMode: EditorMode) => {
    if (newMode === "visual" && mode === "html") {
      if (editorRef.current) { editorRef.current.innerHTML = stripToBodyContent(sourceValue); initialized.current = true }
    }
    if ((newMode === "html" || newMode === "preview") && mode === "visual" && !isCustomHtml) {
      const latest = editorRef.current?.innerHTML ?? value
      setSourceValue(latest)
  ...
  const handleSourceChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    setSourceValue(newVal)
    onChange(newVal)
    setIsCustomHtml(hasCustomMarkup(newVal))
  }
  ...
  const tabStyle = (active: boolean): React.CSSProperties => ({
    ...
    color: active ? "#111827" : "#6b7280", background: active ? "#ffffff" : "transparent",
    border: "none", borderRadius: "5px 5px 0 0", cursor: "pointer",
    borderBottom: active ? "2px solid #111827" : "2px solid transparent", transition: "all 0.15s",
  })

  return (
    <div className="flex flex-col overflow-hidden border rounded-lg border-ui-border-base">
      <div style={{ display: "flex", alignItems: "stretch", flexWrap: "wrap", gap: 0, background: "var(--color-bg-subtle, #f9fafb)", borderBottom: "1px solid var(--color-border-base, #e5e7eb)", padding: "4px 6px 0" }}>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 2, marginRight: 12 }}>
          <button type="button" style={tabStyle(mode === "visual")} onClick={() => handleModeChange("visual")}>Visual</button>
          <button type="button" style={tabStyle(mode === "html")} onClick={() => handleModeChange("html")}>&lt;/&gt; HTML / CSS</button>
          <button type="button" style={tabStyle(mode === "preview")} onClick={() => handleModeChange("preview")}>Preview</button>
        </div>
        {mode === "visual" && !isCustomHtml && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 1, alignItems: "center", paddingBottom: 4 }}>
            {toolbarButtons.map((btn) => (
        ...
        {mode === "visual" && isCustomHtml && (
          <div style={{ display: "flex", alignItems: "center", paddingBottom: 4 }}>
            <span style={{ fontSize: 11, color: "#b45309", fontFamily: "monospace", background: "#fef3c7", padding: "2px 10px", borderRadius: 4, border: "1px solid #fcd34d" }}>
              ⚠ Read-only preview — edit in HTML tab
            </span>
          </div>
        )}
        <span style={{ marginLeft: "auto", fontSize: 11, color: "#9ca3af", padding: "6px 4px 4px", alignSelf: "flex-end" }}>
      ...
      {mode === "visual" && (
        <div
          ref={(el) => {
            editorRef.current = el
            if (el && !initialized.current) { el.innerHTML = stripToBodyContent(sourceValue); initialized.current = true }
          }}
          contentEditable={!isCustomHtml}
          suppressContentEditableWarning
          onInput={handleVisualInput}
          className="min-h-[320px] p-4 text-sm text-ui-fg-base focus:outline-none"
          style={{ lineHeight: "1.7", ...(isCustomHtml ? { cursor: "default", background: "var(--color-bg-subtle, #f9fafb)", userSelect: "text" } : {}) }}
        />
      )}

      {mode === "html" && (
        <textarea value={sourceValue} onChange={handleSourceChange} spellCheck={false}
          style={{ width: "100%", minHeight: 320, padding: "16px", fontFamily: "monospace", fontSize: 13, lineHeight: "1.65", color: "#1e293b", border: "none", outline: "none", resize: "vertical", boxSizing: "border-box", tabSize: 2, whiteSpace: "pre", overflowX: "auto" }}
          placeholder="<!-- Write HTML + inline CSS here -->"
          onKeyDown={(e) => {
            if (e.key === "Tab") {
              e.preventDefault()
              const el = e.currentTarget
              ...
              const newVal = el.value.substring(0, start) + "  " + el.value.substring(end)
              setSourceValue(newVal); onChange(newVal)
      ...
      {mode === "preview" && (
        <iframe key={sourceValue}
          srcDoc={`<!DOCTYPE html><html><head><meta charset="utf-8"/><style>body{font-family:-apple-system,sans-serif;font-size:14px;line-height:1.7;color:#111827;padding:20px 24px;margin:0}h2{font-size:1.4em}h3{font-size:1.2em}p{margin:0.75em 0}a{color:#3b82f6}blockquote{border-left:3px solid #d1d5db;margin:1em 0;padding:0.5em 1em;color:#6b7280}ul,ol{padding-left:1.5em}img{max-width:100%;border-radius:6px}</style></head><body>${sourceValue || "<p style='color:#9ca3af'>Nothing to preview yet.</p>"}</body></html>`}
          style={{ width: "100%", minHeight: 320, border: "none", background: "#fff", display: "block" }}
          sandbox="allow-same-origin" title="Content preview"
        />
      )}

      <div style={{ borderTop: "1px solid var(--color-border-base, #e5e7eb)", padding: "6px 12px", background: "var(--color-bg-subtle, #f9fafb)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Text size="xsmall" className="text-ui-fg-subtle">
          {wordCount} words · ~{Math.ceil(wordCount / 200)} min read
        </Text>
        <Text size="xsmall" className="text-ui-fg-muted">
...
function CoverImageUploader({ value, onChange }: { value: string; onChange: (url: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-y-3">
      ...
      <div className="flex items-center gap-x-2">
        <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleFileUpload} />
        <Button variant="secondary" size="small" isLoading={uploading} type="button" onClick={() => fileInputRef.current?.click()}>
          {uploading ? "Uploading..." : "📁 Upload Image"}
        </Button>
        {value && <Button variant="transparent" size="small" type="button" onClick={() => onChange("")}>Remove</Button>}
      </div>
      {value && (
        <img src={value} alt="Cover preview"
          className="object-cover w-full border rounded-lg aspect-video border-ui-border-base"
...
export default function BlogPage() {
  ...
  const addCategory = async (label: string) => {
    ...
    try {
      ...
      setCategories((prev) => [...prev, data.category])
      toast.success(`"${label}" saved to database`)
    } catch { toast.error("Network error — category not saved") }
  }
  ...
  return (
    <>
      {showCatManager && (
        <CategoryManager categories={categories} loading={loadingCats}
          onAdd={addCategory} onDelete={deleteCategory}
          onClose={() => setShowCatManager(false)} />
      )}
      {view === "list" ? (
        <PostListView
...
function PostListView({
  ...
  const filtered = audienceFilter === "all" ? posts : posts.filter((p) => p.audience === audienceFilter)
    ...
    <div className="flex flex-col p-8 gap-y-4">
      ...
      <div className="flex gap-x-3">
        {[
          { label: "Total",     val: posts.length,                               color: "" },
          { label: "Published", val: posts.filter((p) => p.is_published).length, color: "text-ui-fg-interactive" },
          { label: "For Fans",  val: posts.filter((p) => p.audience === "fans").length, color: "" },
          { label: "For Creators", val: posts.filter((p) => p.audience === "creators").length, color: "" },
        ].map(({ label, val, color }) => (
          <Container key={label} className="flex-1 px-4 py-3">
            <Text size="small" className="text-ui-fg-subtle">{label}</Text>
            <Heading level="h2" className={color}>{val}</Heading>
      ...
      <div className="flex gap-x-1">
        {[
          ...
          <button key={tab.value} type="button" onClick={() => setAudienceFilter(tab.value)}
            style={{
              padding: "6px 16px", fontSize: 13, fontWeight: audienceFilter === tab.value ? 600 : 400,
              color: audienceFilter === tab.value ? "#fff" : "#6b7280",
              background: audienceFilter === tab.value ? "#111827" : "transparent",
          ...
          <Table.Body>
            {loading ? (
              ...
              <Table.Row>
                <Table.Cell style={{ gridColumn: "1 / -1" }}>
                  <div className="flex flex-col items-center py-12 gap-y-2">
                    ...
                    <Button size="small" variant="secondary" type="button" onClick={onNew}>Create your first post</Button>
                  </div>
                </Table.Cell>
              </Table.Row>
            ) : filtered.map((post) => (
              <Table.Row key={post.id} className="cursor-pointer hover:bg-ui-bg-base-hover">
                ...
                <Table.Cell>
                  {post.audience ? (
                    <Badge size="2xsmall" color={post.audience === "fans" ? "blue" : "purple"}>
                      {post.audience === "fans" ? "👥 Fans" : "🎨 Creators"}
                    </Badge>
                  ) : <Text size="small" className="text-ui-fg-muted">—</Text>}
                </Table.Cell>
                <Table.Cell>
                  {post.category
                    ? <Badge size="2xsmall" color="grey">{categories.find((c) => c.value === post.category)?.label ?? post.category.replace(/-/g, " ")}</Badge>
                    : <Text size="small" className="text-ui-fg-muted">—</Text>}
                </Table.Cell>
                <Table.Cell>
                  <div className="flex flex-col gap-y-1">
                    ...
                    {post.is_featured && (
                      <Badge size="2xsmall" color="orange">⭐ Featured</Badge>
                    )}
                  </div>
                </Table.Cell>
                <Table.Cell>
                ...
                <Table.Cell>
                  <div className="flex items-center justify-end gap-x-2">
                    {post.is_published && (
                      <a href={`https://junooni.com/blog/${post.audience}/${post.slug}`} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}>
                        <Button variant="transparent" size="small" type="button"><Eye /></Button>
                      </a>
                    )}
                    <Button variant="transparent" size="small" type="button" onClick={() => onEdit(post)}><PencilSquare /></Button>
                    <Button variant="transparent" size="small" type="button" onClick={() => onDelete(post)}><Trash className="text-ui-fg-error" /></Button>
                  </div>
                </Table.Cell>
              </Table.Row>
            ))}
...
type FormData = {
  title: string; slug: string; excerpt: string; content: string
  cover_image: string; author_name: string; audience: string; category: string; tags: string
  seo_title: string; seo_description: string; is_published: boolean; is_featured: boolean
}
...
function PostFormView({
  ...
  const handleSave = async (publishOverride?: boolean) => {
    if (!form.title.trim()) { toast.error("Title is required"); return }
    if (!form.audience) { toast.error("Please select an audience (Fans or Creators)"); return }
    setSaving(true)
    try {
      const payload = {
        ...form,
  ...
  const audienceLabel = form.audience === "fans" ? "junooni.com/blog/fans/" : form.audience === "creators" ? "junooni.com/blog/creators/" : "junooni.com/blog/[audience]/"
  ...
  return (
    <div className="flex flex-col max-w-4xl p-8 gap-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-x-2">
          <Button variant="secondary" size="small" isLoading={saving} type="button" onClick={() => handleSave(false)}>Save Draft</Button>
          <Button size="small" isLoading={saving} type="button" onClick={() => handleSave(true)}>
            {form.is_published ? "Save & Keep Published" : "Publish"}
          </Button>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="flex flex-col col-span-2 gap-y-4">
          <Container className="flex flex-col p-6 gap-y-4">
            <div className="flex flex-col gap-y-1">
              <Text size="small" weight="plus">Title *</Text>
              <Input value={form.title} onChange={handleTitleChange} placeholder="e.g. How Tanishk Bagchi launched his merch on JUNOONI" />
            </div>
            <div className="flex flex-col gap-y-1">
              <Text size="small" weight="plus">Slug</Text>
              <Input value={form.slug} onChange={set("slug")} placeholder="auto-generated-from-title" className="font-mono text-sm" />
              <Text size="xsmall" className="text-ui-fg-subtle">{audienceLabel}{form.slug || "your-slug"}</Text>
            </div>
            <div className="flex flex-col gap-y-1">
              <Text size="small" weight="plus">Excerpt</Text>
              <Textarea value={form.excerpt} onChange={set("excerpt")} placeholder="Short summary shown in blog listing (160 chars recommended)" rows={2} />
            </div>
            <div className="flex flex-col gap-y-1">
              <Text size="small" weight="plus">Content</Text>
              <RichTextEditor value={form.content} onChange={(html) => setForm((f) => ({ ...f, content: html }))} />
          ...
          <Container className="flex flex-col p-6 gap-y-4">
            ...
            <div className="flex flex-col gap-y-1">
              <Text size="small" weight="plus">Meta Description</Text>
              <Textarea value={form.seo_description} onChange={set("seo_description")} placeholder="Defaults to excerpt (max 160 chars)" rows={2} />
              <Text size="xsmall" className={form.seo_description.length > 160 ? "text-ui-fg-error" : "text-ui-fg-subtle"}>
                {form.seo_description.length}/160 characters
              </Text>
            </div>
        ...
        <div className="flex flex-col gap-y-4">
          <Container className="flex flex-col p-6 gap-y-4">
            ...
            <div className="pt-3 border-t border-ui-border-base">
              <div className="flex items-center justify-between">
                <div>
                  <Text size="small" weight="plus">Featured Post</Text>
                  <Text size="xsmall" className="text-ui-fg-subtle">Show in featured section</Text>
                </div>
                <Switch checked={form.is_featured} onCheckedChange={(v) => setForm((f) => ({ ...f, is_featured: v }))} />
              </div>
              {form.is_featured && (
                <div style={{ marginTop: 8, padding: "6px 10px", background: "#fef3c7", borderRadius: 6, border: "1px solid #fcd34d" }}>
                  <Text size="xsmall" style={{ color: "#b45309" }}>
                    ⭐ Only one post per audience should be featured at a time
                  </Text>
                </div>
              )}
            </div>
          ...
          <Container className="flex flex-col p-6 gap-y-4">
            ...
            <div className="flex flex-col gap-y-1">
              ...
              <Select
                key={`audience-${form.audience}`}
                value={form.audience}
                onValueChange={(v) => setForm((f) => ({ ...f, audience: v }))}
              >
                <Select.Trigger><Select.Value placeholder="Select audience" /></Select.Trigger>
                <Select.Content>
            ...
            <div className="flex flex-col gap-y-1">
              <div className="flex items-center justify-between">
                <Text size="small" weight="plus">Category</Text>
                <button type="button" onClick={onManageCategories}
                  style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "var(--color-fg-interactive, #60a5fa)", padding: 0 }}>
                  + Manage
                </button>
              </div>
              <Select
            ...
            <div className="flex flex-col gap-y-1">
              <Text size="small" weight="plus">Tags</Text>
              <Input value={form.tags} onChange={set("tags")} placeholder="creator, merch, music (comma-separated)" />
            </div>
          </Container>

          <Container className="flex flex-col p-6 gap-y-4">
            <Heading level="h3">Cover Image</Heading>
            <CoverImageUploader value={form.cover_image} onChange={(url) => setForm((f) => ({ ...f, cover_image: url }))} />
          </Container>
        </div>
      </div>
    </div>
