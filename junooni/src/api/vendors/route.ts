...
// // ─── Shared schema for vendor fields (used in both POST and PUT) ──────────────
// // Keeping this separate avoids duplication and makes future field additions
...
//   // Store mode flags — both can be true at the same time
//   // sell_on_marketplace: creator's products appear on junooni.com/store/handle
//   // sell_on_own_store:   creator gets a branded storefront at their own domain
//   sell_on_marketplace: z.boolean().optional().default(true),
//   sell_on_own_store: z.boolean().optional().default(false),
// })

// // POST body adds the required `admin` field that doesn't apply to updates
...
// // ─── GET /vendors — list vendors or fetch single by vendor_id query param ────
...
// // ─── DELETE /vendors?vendor_id=xxx — hard delete a vendor (admin only) ───────
