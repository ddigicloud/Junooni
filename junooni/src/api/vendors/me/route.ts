...
async function resolveVendorId(
  ...
  // ✅ read vendor_id column directly — no join needed
...
export const GET = async (
  ...
  // Fetch full vendor — plan fields and marketplace status are columns on the vendor entity
  ...
  return res.json({
    vendor: {
      ...
      marketplace_status: vendor.marketplace_status ?? "none",
      admins: admins ?? [],
    }
  })
}
...
export const UpdateVendorSchema = z.object({
  // Marketplace access is admin-controlled only — creator cannot set this directly.
  // Use POST /vendors/marketplace-application to apply instead.
  sell_on_marketplace: z.never().optional(),
...
async function updateVendorForMe(
  ...
  const { admins, sell_on_marketplace, ...vendorFields } = body

  // Silently strip sell_on_marketplace — creator cannot set this directly.
  // It is only set by admin approval via POST /admin/vendors/:id/marketplace-application
  if (sell_on_marketplace !== undefined) {
    console.warn(`Vendor ${vendorId} attempted to directly set sell_on_marketplace — blocked.`)
  }
