...
export async function POST(
  ...
  // FIX: cast to any — req.body is typed as unknown in Medusa 2.15
  // Runtime validation below ensures the shape is correct
  ...
  const vendorOrderIds = vendorAdmin?.vendor?.orders?.map((o: any) => o.id) || []
  ...
  try {
    ...
    const { result: fulfillment } = await createOrderFulfillmentWorkflow(
      ...
      input: {
        ...
        metadata: {
          ...metadata,
          vendor_id: (vendorAdmin as any).vendor.id,
          vendor_admin_id: req.auth_context.actor_id,
          fulfillment_source: "vendor_portal",
          created_at: new Date().toISOString()
        },
