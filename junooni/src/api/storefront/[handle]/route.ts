...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  // Full published product id set — so collection counts aren't limited to the current page
  let allVendorProductIds: string[] = products.map((p: any) => p.id)
  try {
    ...
    if (allIds?.length) allVendorProductIds = allIds.map((p: any) => p.id)
  } catch (err) {
    console.error("[store-front] id-only count query failed:", err)
  }
  const idSet = new Set(allVendorProductIds)
  ...
  const vendorCollections = rawVendorCollections
    ...
    .map((c: any) => ({
      ...c,
      product_count: (c.product_ids ?? []).filter((id: string) => idSet.has(id)).length,
    }))
