...
export async function POST(
  ...
  try {
    ...
    // ✅ FIXED: Enhanced query to fetch HSN codes from variants and products
    const { data: orders } = await query.graph({
      entity: "order",
      fields: [
        "id",
        ...
        // ✅ CRITICAL: Fetch items with complete tax and HSN data
        "items.*",
        "items.tax_lines.*",
        "items.variant.*",
        ...
        // ✅ Fetch shipping methods with tax data
        "shipping_methods.*",
    ...
    let pdfBuffer
    try {
      console.log('=== DETAILED DEBUGGING FOR INVOICE GENERATION ===')
      ...
      const safeTaxTotal = extractNumeric(order.tax_total)
      const safeShippingTotal = extractNumeric(order.shipping_total)
      const safeShippingSubtotal = extractNumeric(order.shipping_subtotal || order.shipping_methods?.[0]?.subtotal || 0)
      const safeShippingTaxTotal = extractNumeric(order.shipping_tax_total || order.shipping_methods?.[0]?.tax_total || 0)
      const safeDiscountTotal = extractNumeric(order.discount_total)
      const safeTotal = extractNumeric(order.total)
      
      console.log('✅ EXTRACTED ORDER VALUES:', {
        subtotal: safeSubtotal,
        tax_total: safeTaxTotal,
        shipping_total: safeShippingTotal,
        shipping_subtotal: safeShippingSubtotal,
        shipping_tax_total: safeShippingTaxTotal,
        discount_total: safeDiscountTotal,
        total: safeTotal
      })
      ...
      const correctOrderStructure = {
        ...
        // Shipping values with tax breakdown
        shipping_total: safeShippingTotal,
        shipping_subtotal: safeShippingSubtotal,
        shipping_tax_total: safeShippingTaxTotal,
        ...
        shipping_methods: order.shipping_methods?.map((method: any) => ({
          ...
          name: "Standard Shipping",
          total: safeShippingTotal,
          subtotal: safeShippingSubtotal,
          tax_total: safeShippingTaxTotal
        }],
      ...
      // ✅ FIXED: Process items with BigNumber conversion AND HSN code extraction
      const correctItems = (order.items || []).map((item: any) => {
        const quantity = extractNumeric(item.quantity)
        const unitPrice = extractNumeric(item.unit_price)
        const total = extractNumeric(item.total)
        const taxTotal = extractNumeric(item.tax_total || 0)
        
        // ✅ CRITICAL: Extract HSN code from variant or product
        const hsCode = item.variant?.product?.hs_code || 
                       item.variant?.hs_code || 
                       item.product?.hs_code || 
                       'N/A'
        
        console.log(`🏷️ Item "${item.title}" HSN code:`, hsCode)
        console.log(`💰 Item "${item.title}" values:`, {
          quantity,
        ...
        return {
          ...
          total: total,
          tax_total: taxTotal,
          hs_code: hsCode, // ✅ Pass HSN code explicitly
          ...
          // ✅ Pass tax lines for accurate GST calculation
      ...
      console.log('Order:', JSON.stringify({
        subtotal: correctOrderStructure.subtotal,
        tax_total: correctOrderStructure.tax_total,
        shipping_total: correctOrderStructure.shipping_total,
        shipping_subtotal: correctOrderStructure.shipping_subtotal,
        shipping_tax_total: correctOrderStructure.shipping_tax_total,
        discount_total: correctOrderStructure.discount_total,
        total: correctOrderStructure.total,
        currency_code: correctOrderStructure.currency_code
      }, null, 2))
