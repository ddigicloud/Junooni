...
//     // Default marketplace channel (or missing info) → Junooni branding
//     if (!sales_channel_id || sales_channel_id === defaultChannelId || !vendor_id) {
//       return new StepResponse({ storeLogo: null, storeName: null })
//     }
...
//     if (!storeLogo) {
//       // Vendor has no store / no logo set → fall back to Junooni branding
...
//       (vendorStore.settings as any)?.accent_color || null
