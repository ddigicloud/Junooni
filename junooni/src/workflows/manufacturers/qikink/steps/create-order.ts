...
// import { getQikinkTokenStep } from "./get-token"
// import { buildQikinkPayloadStep } from "./build-payload"
// import { submitQikinkOrderStep } from "./submit-order"
// import { fetchQikinkTrackingStep } from "./fetch-tracking"
...
// /**
//  * Main step to create Qikink order
//  * This orchestrates the sub-steps: token, payload, submit, tracking
//  */
...
//     console.log(`📦 Items to sync: ${input.items.length}`)

//     try {
//       // Step 1: Get Qikink access token
//       const { accessToken } = await getQikinkTokenStep.invoke(
//         {},
//         { container }
//       )

//       // Step 2: Build Qikink payload from order and items
//       const { payload, shortOrderId } = await buildQikinkPayloadStep.invoke(
//         {
//           order: input.order,
//           items: input.items,
//         },
...
//       // Step 3: Submit order to Qikink
//       const { result } = await submitQikinkOrderStep.invoke(
//         {
//           payload,
//           accessToken,
//         },
...
//       // Step 4: Fetch tracking information
//       let trackingInfo = null
//       try {
//         const tracking = await fetchQikinkTrackingStep.invoke(
//           {
//             qikinkOrderId: result.order_id,
//             accessToken,
//           },
//           { container }
//         )
...
//       } catch (trackingError) {
//         console.warn("⚠️ Could not fetch tracking immediately:", trackingError)
//         console.log("💡 Tracking will be available once Qikink processes the order")
//       }

//       const output: QikinkOrderOutput = {
//         success: true,
//         order_id: result.order_id,
//         order_number: result.number || shortOrderId,
...
//         has_tracking: !!trackingInfo?.tracking_number,
//       })

//       // Store Qikink order info in order metadata for future reference
//       return new StepResponse(output, {
//         qikink_order_id: output.order_id,
//         qikink_order_number: output.order_number,
//         medusa_order_id: input.order.id,
//       })
//     } catch (error) {
//       console.error("❌ Failed to create Qikink order:", error)
//       throw error
//     }
//   },
//   async (compensation, { container }) => {
//     // Compensation logic if order creation fails
//     // You might want to:
//     // 1. Cancel the Qikink order if it was partially created
//     // 2. Log the failure
//     // 3. Send notification to admin
//     console.log("🔄 Compensating Qikink order creation...")
    
//     if (compensation?.qikink_order_id) {
//       console.log(`⚠️ Qikink order ${compensation.qikink_order_id} may need manual cancellation`)
//       // TODO: Implement Qikink order cancellation API call if available
//     }
//   }
// )
