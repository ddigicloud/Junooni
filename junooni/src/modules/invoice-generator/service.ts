...
class InvoiceGeneratorService extends MedusaService({
  ...
  private async createInvoiceContent(
    ...
    const shipFromAddress = firstVendorItem.ship_from
      ...
      // ✅ Get vendor GST number from vendor_data if available
      const vendorGSTIN = firstVendorItem.vendor_data?.GSTIN || null
      console.log('🔍 Vendor GSTIN:', vendorGSTIN)
      ...
      const gstBreakdown = this.calculateGSTBreakdown(params.order, vendorItems, shipFromAddress)
...
${params.order.shipping_address.city || ''} - ${params.order.shipping_address.postal_code || ''} ${params.order.shipping_address.province || ''}, ${params.order.shipping_address.country_code?.toUpperCase() || ''}` : 
        ...
        // GSTIN Number section - conditional based on fulfillment type
        {
          text: `GSTIN Number: ${
            firstVendorItem.fulfillment_type === 'junooni' 
              ? 'GSTjunooni1234' 
              : (vendorGSTIN || 'N/A')
          }`,
          style: 'gstin',
  ...
  private calculateGSTBreakdown(order: any, items: any[], shipFromAddress: any) {
    ...
    // ✅ FIXED: Compare seller's state (ship_from) with buyer's state (shipping address)
    const isIntraState = this.checkIfIntraState(shipFromAddress, order.shipping_address)
  ...
  private checkIfIntraState(shipFromAddress: any, shippingAddress: any): boolean {
    // ✅ FIXED: Compare SELLER's state (ship_from) with BUYER's state (shipping address)
    const sellerState = shipFromAddress?.province?.toLowerCase()
    const buyerState = shippingAddress?.province?.toLowerCase()
    
    // ✅ FIXED: Ensure we return a proper boolean
    const result = !!(sellerState && buyerState && sellerState === buyerState)
    console.log('🏠 State comparison (Seller vs Buyer):', { 
      sellerState, 
      buyerState, 
      result,
      isIntraState: result ? 'YES - Same state (CGST + SGST)' : 'NO - Different states (IGST)'
    })
    return result
  }
