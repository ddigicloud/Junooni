...
class InvoiceGeneratorService extends MedusaService({
  ...
  private async createInvoiceContent(
    ...
    // ✅ FIXED: PRE-FORMAT ITEM VALUES WITH CORRECT GST CALCULATION
    const formattedItems = await Promise.all(
      safeItems.map(async item => {
        console.log(`\n🔍 Processing item: "${item.title}"`)
        
        // ✅ Get HSN code directly from the item (passed from route)
        const hsCode = item.hs_code || 'N/A'
        console.log(`🏷️ HSN Code:`, hsCode)
        
        // ✅ CRITICAL FIX: Medusa's unit_price and total are TAX-INCLUSIVE
        // We need to extract the tax-exclusive amounts
        ...
        console.log(`💰 Price breakdown for "${item.title}":`, {
          quantity,
          tax_inclusive_unit_price: taxInclusiveUnitPrice,
          tax_inclusive_total: taxInclusiveTotal,
          item_tax_total: itemTaxTotal,
          tax_exclusive_unit_price: taxExclusiveUnitPrice,
          tax_exclusive_total: taxExclusiveTotal
        })
        ...
        if (itemTaxTotal > 0) {
          if (isIntraState) {
            // Intra-state: CGST + SGST = Total Tax (split equally)
            itemCgstAmount = itemTaxTotal / 2
            itemSgstAmount = itemTaxTotal / 2
          } else {
        ...
        console.log(`📊 GST breakdown for "${item.title}":`, {
          isIntraState,
          cgst_amount: itemCgstAmount,
          sgst_amount: itemSgstAmount,
          igst_amount: itemIgstAmount
        ...
        return {
          title: item.title || 'Unknown Item',
          hs_code: hsCode, // ✅ Now properly passed
          sale_price: await this.formatAmount(taxExclusiveUnitPrice, params.order.currency_code), // ✅ Tax-exclusive price
          quantity: quantity.toString(),
          cgst_rate: isIntraState ? `${gstBreakdown.cgst.rate.toFixed(2)}` : '0.00',
          cgst_amount: await this.formatAmount(itemCgstAmount, params.order.currency_code), // ✅ Now positive
          sgst_rate: isIntraState ? `${gstBreakdown.sgst.rate.toFixed(2)}` : '0.00',
          sgst_amount: await this.formatAmount(itemSgstAmount, params.order.currency_code), // ✅ Now positive
          igst_rate: !isIntraState ? `${gstBreakdown.igst.rate.toFixed(2)}` : '0.00',
          igst_amount: await this.formatAmount(itemIgstAmount, params.order.currency_code),
          net_amount: await this.formatAmount(taxInclusiveTotal, params.order.currency_code) // ✅ Tax-inclusive total
        }
      })
    )

    // ✅ FIXED: Add service charges with proper shipping GST breakdown
    const serviceCharges = []
    
    // Shipping Charges - only if shipping cost > 0
    if (safeOrderTotals.shipping_total > 0) {
      // ✅ CRITICAL: Get shipping tax from order data
      // Medusa stores shipping amounts as tax-inclusive
      const shippingTotal = this.extractNumeric(params.order.shipping_total || params.order.shipping_methods?.[0]?.total || 0)
      const shippingTaxTotal = this.extractNumeric(params.order.shipping_tax_total || params.order.shipping_methods?.[0]?.tax_total || 0)
      
      // ✅ Calculate tax-exclusive shipping amount
      const shippingSubtotal = shippingTotal - shippingTaxTotal
      ...
      // Calculate shipping tax components
      const isIntraState = Boolean(gstBreakdown.isIntraState)
      let shippingCgstAmount = 0
      let shippingSgstAmount = 0
      let shippingIgstAmount = 0
      ...
      console.log('🚚 Shipping GST:', {
        isIntraState,
        cgst: shippingCgstAmount,
        sgst: shippingSgstAmount,
        igst: shippingIgstAmount
      })
      
      serviceCharges.push({
        title: 'Shipping Charge',
        hs_code: '996819',
        sale_price: await this.formatAmount(shippingSubtotal, params.order.currency_code), // ✅ Tax-exclusive
        quantity: '',
        cgst_rate: (shippingTaxTotal > 0 && isIntraState) ? `${gstBreakdown.cgst.rate.toFixed(2)}` : '0.00',
        cgst_amount: await this.formatAmount(shippingCgstAmount, params.order.currency_code),
        sgst_rate: (shippingTaxTotal > 0 && isIntraState) ? `${gstBreakdown.sgst.rate.toFixed(2)}` : '0.00',
        ...
        igst_rate: (shippingTaxTotal > 0 && !isIntraState) ? `${gstBreakdown.igst.rate.toFixed(2)}` : '0.00',
        igst_amount: await this.formatAmount(shippingIgstAmount, params.order.currency_code),
        net_amount: await this.formatAmount(shippingTotal, params.order.currency_code) // ✅ Tax-inclusive
      })
    }
    ...
    // Create detailed items table body with proper GST column headers
    const itemsTableBody = [
      // First header row - main headers
      [
        { text: 'Product\nDescription', style: 'tableHeader', alignment: 'left', rowSpan: 2 },
...
${params.order.billing_address.country_code?.toUpperCase() || ''}${params.order.billing_address.phone ? `\nPhone: ${params.order.billing_address.phone}` : ''}` : 
 ...
 ${params.order.shipping_address.country_code?.toUpperCase() || ''}${params.order.shipping_address.phone ? `\nPhone: ${params.order.shipping_address.phone}` : ''}` : 
        ...
        // Items Table with proper GST headers and NET AMT visible
        {
          margin: [0, 20, 0, 0],
          table: {
            headerRows: 2,
  ...
  private async formatAmount(amount: number, currency: string): Promise<string> {
    ...
    try {
      // Use simple number formatting with Rs. prefix
      const formattedNumber = Number(amount).toLocaleString('en-IN', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      })
