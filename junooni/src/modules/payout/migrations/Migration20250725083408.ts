...
export class Migration20250725083408 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "payout" ("id" text not null, "vendor_id" text not null, "order_id" text not null, "amount" integer not null, "type" text check ("type" in ('earning', 'payout', 'adjustment', 'refund')) not null, "status" text check ("status" in ('pending', 'processing', 'completed', 'failed', 'cancelled')) not null default 'pending', "reason" text not null, "notes" text null, "payout_period" text null, "scheduled_payout_date" timestamptz null, "payment_method" text check ("payment_method" in ('bank_transfer', 'paypal', 'razorpay', 'manual')) null, "external_reference_id" text null, "processor_response" text null, "payout_total" integer null, "current_balance" integer not null default 0, "pending_balance" integer not null default 0, "total_earned" integer not null default 0, "total_paid" integer not null default 0, "total_pending_payout" integer not null default 0, "total_orders" integer not null default 0, "avg_order_value" integer not null default 0, "minimum_payout_amount" integer not null default 1000, "payout_schedule" text check ("payout_schedule" in ('weekly', 'biweekly', 'monthly')) not null default 'weekly', "preferred_payment_method" text check ("preferred_payment_method" in ('bank_transfer', 'paypal', 'razorpay')) not null default 'bank_transfer', "last_payout_at" timestamptz null, "last_earning_at" timestamptz null, "next_payout_date" timestamptz null, "is_payout_enabled" boolean not null default true, "hold_payouts" boolean not null default false, "hold_reason" text null, "payout_batch_id" text not null, "processed_at" timestamptz null, "created_by" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "payout_pkey" primary key ("id"));`);
    ...
    this.addSql(`create table if not exists "payout_details" ("id" text not null, "product_id" text not null, "amount" integer not null, "tax_amount" integer not null default 0, "tax_type" text check ("tax_type" in ('igst', 'sgst-cgst')) not null default 'igst', "tds_percentage" integer not null default 0, "tds_amount" integer not null default 0, "type" text check ("type" in ('earning', 'payout', 'adjustment', 'refund')) not null, "fulfillment_type" text check ("fulfillment_type" in ('creator_fulfillment', 'junooni_fulfillment')) null, "cost_price" integer null, "commission_rate" integer null, "selling_price" integer null, "status" text check ("status" in ('pending', 'processing', 'completed', 'failed', 'cancelled')) not null default 'pending', "reason" text not null, "notes" text null, "payout_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "payout_details_pkey" primary key ("id"));`);
