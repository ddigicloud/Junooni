...
export default async function googleMerchantSyncJob(container: MedusaContainer) {
  ...
  while (true) {
    const { data: products } = await query.graph({
      entity: "product",
      // status: "published" narrows at the DB level — the mapper's own
      // published + Default Sales Channel checks are the real gate below
      filters: { status: "published" },
      fields: [
        "id", "title", "handle", "description", "status",
        "thumbnail", "metadata",
    ...
    const eligibleProducts = products.filter((p: any) =>
      (p.sales_channels || []).some((sc: any) => sc.name === "Default Sales Channel")
    )
    totalSkipped += products.length - eligibleProducts.length

    const allMerchantProducts = eligibleProducts.flatMap(mapProductToMerchantProducts)
  ...
  logger.info(
    `[GoogleMerchant] Scheduled sync done. Synced: ${totalSynced}, Failed: ${totalFailed}, Skipped (no Default Sales Channel): ${totalSkipped}`
  )
}
