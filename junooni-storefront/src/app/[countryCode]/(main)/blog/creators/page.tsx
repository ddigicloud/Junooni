...
async function getPosts(): Promise<{ featured: Post | null; posts: Post[] }> {
  try {
    const res = await fetch(`${BACKEND}/store/blog/creators?limit=20`, {
      next: { revalidate: 60 },
...
export const metadata: Metadata = {
  title: "For Creators | JUNOONI Blog",
  description: "Launch guides, merch strategy, platform tips and creator success stories on JUNOONI.",
  openGraph: { title: "JUNOONI Blog — For Creators", url: "https://junooni.com/blog/creators" },
}

export default async function CreatorsBlogPage() {
          ...
          <>
            ...
            {posts.length > 0 && (
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {posts.map((post) => (
                  <Link key={post.id} href={`/blog/creators/${post.slug}`} className="flex flex-col group">
