...
import React, { useState, useEffect } from "react"
import {
  Search, ArrowLeft, ChevronDown, ChevronLeft, ChevronRight,
  X, Users, Store, Package, ShoppingBag, Truck,
  Film, Shirt, MoreHorizontal, Palette, Laugh, Award,
} from "lucide-react"
import { retriveVendorsFollowers } from "@lib/data/vendors"
import { followerList, Addfollower, deletefollower, retrieveCustomer } from "@lib/data/customer"
import { assets } from "@assets/assets"
import { toast } from "react-toastify"
import Link from "next/link"
...
// ── Props: vendors fetched server-side, passed in ─────────────────────────────
interface Props {
  initialVendors: Vendor[]
}

const VendorsDiscoveryPage = ({ initialVendors }: Props) => {
  ...
  // KEY FIX: initialise directly from prop — no useEffect fetch needed
  const [allVendors] = useState<Vendor[]>(initialVendors)
  ...
  // ── Build categories from vendors (fast — no API call) ─────────────────────
  useEffect(() => {
    if (allVendors.length === 0) return
    const iconMap: Record<string, React.ReactNode> = {
      default: <Store size={18} />, store: <Store size={18} />,
      cinema: <Film size={18} />, fashion: <Shirt size={18} />,
      other: <MoreHorizontal size={18} />, art: <Palette size={18} />,
      comedy: <Laugh size={18} />, influencer: <Award size={18} />,
      electronics: <Package size={18} />, food: <ShoppingBag size={18} />,
      delivery: <Truck size={18} />,
    }
    const categorySet = new Set<string>()
    allVendors.forEach((v) => { if (v.creator_category) categorySet.add(v.creator_category) })
    setCategories([
      { id: "all", name: "All Categories", icon: <Store size={18} /> },
      ...Array.from(categorySet).map((cat) => ({
        id: cat.toLowerCase(),
  ...
  // ── Follower counts — parallel fetch ───────────────────────────────────────
  useEffect(() => {
    if (allVendors.length === 0) return
    Promise.all(
      allVendors.map(async (vendor) => {
        try {
          const result = await retriveVendorsFollowers(vendor.id)
          return { id: vendor.id, count: Array.isArray(result?.follow) ? result.follow.length : 0 }
        } catch {
          return { id: vendor.id, count: 0 }
        }
      })
    ).then((results) => {
      const counts: Record<string, number> = {}
  ...
  useEffect(() => {
    ...
    followerList()
      .then((data) => {
        if (data?.follow?.creators && Array.isArray(data.follow.creators)) {
          const states: Record<string, boolean> = {}
          data.follow.creators.forEach((item: any) => {
            if (item.vendor?.id) states[item.vendor.id] = true
          })
          setFollowedVendors(states)
        }
      })
      .catch(() => {})
  }, [currentCustomer])

  // ── Filter vendors — client-side, instant ─────────────────────────────────
  useEffect(() => {
    let filtered = allVendors
    if (searchQuery) {
    ...
    if (activeCategory !== "all") {
      filtered = filtered.filter(
        (v) => v.creator_category?.toLowerCase() === activeCategory.toLowerCase()
      )
    }
    setFilteredVendors(filtered)
    setCurrentPage(1)
  ...
  // ── Follow toggle ──────────────────────────────────────────────────────────
  const handleFollowToggle = async (vendorId: string) => {
    if (!currentCustomer) { toast.error("Please log in to follow creator"); return }
    setIsLoadingFollow((prev) => ({ ...prev, [vendorId]: true }))
    try {
      const isFollowed = followedVendors[vendorId]
      if (isFollowed) {
        await deletefollower(vendorId)
        setFollowedVendors((prev) => ({ ...prev, [vendorId]: false }))
        setFollowerCounts((prev) => ({ ...prev, [vendorId]: Math.max(0, (prev[vendorId] || 0) - 1) }))
      } else {
        await Addfollower(vendorId)
        setFollowedVendors((prev) => ({ ...prev, [vendorId]: true }))
        setFollowerCounts((prev) => ({ ...prev, [vendorId]: (prev[vendorId] || 0) + 1 }))
      }
    } catch {
      toast.error("Failed to update follow status")
    } finally {
  ...
  const VendorCard = ({ vendor }: { vendor: Vendor }) => {
    const { id, name, handle, logo, coverphoto, creator_category, creator_bio } = vendor
    const isFollowed = followedVendors[id] || false
    const isFollowLoading = isLoadingFollow[id] || false
    const followerCount = followerCounts[id] || 0

    const handleCardClick = (e: React.MouseEvent) => {
      e.preventDefault()
      // Instant URL update → loading.tsx shows immediately
      router.push(`/creator/${handle}`)
    }

    return (
      <Link href={`/creator/${handle}`} prefetch={true} onClick={handleCardClick}>
        <motion.div
          className="relative flex flex-col h-full overflow-hidden transition-all duration-300 bg-white rounded-lg shadow-sm cursor-pointer hover:shadow-md group"
          whileHover={{ y: -4 }}
        >
          ...
          <div className="relative h-32 bg-gradient-to-br from-orange-400 to-orange-600">
            {coverphoto && (
              <img src={coverphoto} alt={name} className="object-cover w-full h-full" />
            )}
          </div>
          ...
          <div className="relative z-10 flex justify-center -mt-12">
            <div className="w-20 h-20 overflow-hidden bg-white border-4 border-white rounded-full shadow-md">
              <img src={logo || avatarImage.src} alt={name} className="object-cover w-full h-full" />
            </div>
          </div>
          ...
          <div className="flex flex-col flex-1 px-4 pt-3 pb-4 text-center">
            <div className="flex-1">
              ...
              {creator_bio && (
                <p className="mb-3 text-sm text-gray-600 line-clamp-2">{creator_bio}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center justify-between pt-3 border-t">
                ...
                <button
                  onClick={(e) => { e.preventDefault(); handleFollowToggle(id) }}
                  disabled={isFollowLoading || !currentCustomer}
                  className={`group px-3 py-1 text-xs font-medium rounded-full transition ${
                    isFollowed
                      ? "bg-gray-100 text-gray-700 hover:bg-gray-200"
                      : "bg-[#e65100] text-white hover:bg-[#d84315]"
                  } ${isFollowLoading || !currentCustomer ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  {isFollowLoading ? "..." : !currentCustomer ? "Follow" : isFollowed ? "Following" : "Follow"}
                </button>
              </div>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="px-4 py-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              {searchQuery && (
                <button className="absolute text-gray-400 transform -translate-y-1/2 right-3 top-1/2 hover:text-gray-500" onClick={() => setSearchQuery("")}>
                  <X size={16} />
                </button>
              )}
            </div>
            <button
              className="flex items-center gap-2 p-2 px-4 bg-gray-100 rounded-full"
              onClick={() => setIsFiltersOpen(!isFiltersOpen)}
            >
              Filter
      ...
      {isFiltersOpen && (
        <div className="bg-white border-t border-b shadow-sm">
          <div className="px-4 py-4 mx-auto">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-medium">Filter by Category</h3>
              <button className="text-sm text-[#e65100] hover:underline" onClick={() => { setActiveCategory("all"); setSearchQuery("") }}>
                Reset Filters
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              {categories.map((category) => (
                <button
                  key={category.id}
                  className={`flex items-center px-3 py-1.5 rounded-full text-sm font-medium transition ${
                    activeCategory === category.id ? "bg-[#e65100] text-white" : "bg-gray-100 text-gray-800 hover:bg-gray-200"
                  }`}
                  onClick={() => setActiveCategory(category.id)}
                >
                  <span className="mr-1.5">{category.icon}</span>
      ...
      <main className="px-1 py-6 sm:px-4">
        {filteredVendors.length > 0 ? (
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-8">
                <button
                  ...
                  disabled={currentPage === 1}
                  className={`flex items-center gap-1 px-3 py-2 rounded-lg border transition ${currentPage === 1 ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-white text-gray-700 hover:bg-gray-50"}`}
                >
                  <ChevronLeft size={16} />
                  <span className="hidden sm:inline">Previous</span>
                </button>
                  ...
                  {getPageNumbers().map((pageNum, index) =>
                    pageNum === "..." ? (
                      ...
                      <button
                        key={pageNum}
                        onClick={() => handlePageChange(pageNum as number)}
                        className={`px-3 py-2 rounded-lg border transition ${currentPage === pageNum ? "bg-[#e65100] text-white border-[#e65100]" : "bg-white text-gray-700 hover:bg-gray-50"}`}
                      >
                        {pageNum}
                      </button>
                    )
                ...
                <button
                  onClick={() => handlePageChange(currentPage + 1)}
                  disabled={currentPage === totalPages}
                  className={`flex items-center gap-1 px-3 py-2 rounded-lg border transition ${currentPage === totalPages ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-white text-gray-700 hover:bg-gray-50"}`}
                >
                  <span className="hidden sm:inline">Next</span>
                  <ChevronRight size={16} />
                </button>
          ...
          <div className="p-8 text-center bg-white rounded-lg shadow-sm">
            <div className="mb-4 text-gray-400"><Search size={64} className="mx-auto" /></div>
            <h3 className="mb-2 text-xl font-medium">No creators found</h3>
            <p className="mb-6 text-gray-600">
              {searchQuery || activeCategory !== "all" ? "Try adjusting your search terms or filters" : "No creators are available at the moment"}
            </p>
            {(searchQuery || activeCategory !== "all") && (
              <button onClick={() => { setActiveCategory("all"); setSearchQuery("") }} className="px-4 py-2 text-white bg-[#e65100] rounded-md hover:bg-[#d84315] transition">
                Clear All Filters
              </button>
            )}
          </div>
