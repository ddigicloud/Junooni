...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    ...
    if (!collection) {
      return {
        title: "Collection Not Found | Junooni",
        description: "The requested collection could not be found.",
      }
    }
    return {
      title: `${collection.title} | Junooni`,
      description:
        collection.description ||
        `Shop ${collection.title} merchandise from your favourite creators on Junooni.`,
    }
  } catch (error) {
    return {
      title: "Error | Junooni",
...
export default async function CollectionPage({ searchParams, params }: Props) {
  ...
  const { sortBy, page, category, vendors, colors, price } = resolvedSearchParams
  ...
  try {
    ...
    const collectionSchema = {
      ...
      "description":
        collection.description ||
        `${collection.title} — exclusive creator merch on Junooni`,
      "url": `https://junooni.com/in/collections/${resolvedParams.handle}`,
    }
    ...
    const breadcrumbSchema = {
      ...
      "itemListElement": [
        ...
        },
        {
          ...
          "position": 3,
          "name": collection.title,
          "item": `https://junooni.com/in/collections/${resolvedParams.handle}`,
        },
      ],
    }
