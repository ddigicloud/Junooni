...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    ...
    const title = productCategory.name
    const description =
      productCategory.description || `Shop ${title} merchandise on Junooni.`
    return {
      title: `${title} | Junooni`,
      description,
      alternates: {
        canonical: `https://junooni.com/in/categories/${params.category.join("/")}`,
      },
    }
  } catch (error) {
    return {
...
export default async function CategoryPage({ searchParams, params }: Props) {
  const { sortBy, page, vendors, colors, price, collections } = searchParams
  ...
  try {
    ...
    const categorySchema = {
      ...
      "description":
        ...
        `Shop ${productCategory.name} merchandise on Junooni`,
      "url": `https://junooni.com/in/categories/${params.category.join("/")}`,
    }
    ...
    const breadcrumbSchema = {
      ...
      "itemListElement": [
        ...
        },
        {
          ...
          "position": 3,
          "name": productCategory.name,
          "item": `https://junooni.com/in/categories/${params.category.join("/")}`,
        },
      ],
    }
