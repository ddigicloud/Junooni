...
export default function ClientCollectionShell({
  collection,
  allProducts,
  categoriesData,
  vendorsData,
  ...
  const [selectedCategories, setSelectedCategories] = useState<string[]>(initialCategoryHandles)
  ...
  useEffect(() => {
    ...
    setSelectedCategories(sp.get("category")?.split(",").filter(Boolean) || [])
  ...
  // Vendors from collection products
  const collectionVendors = useMemo(() => {
  ...
  // Collections in this collection's products
  const collectionCollections = useMemo(() => {
    const map = new Map<string, any>()
    allProducts.forEach((p) => {
  ...
  }, [allProducts, selectedCategories, selectedVendors, selectedColors, minPrice, maxPrice, sortBy])
  ...
  // ── setQueryParams — instant state + URL update ───────────────────────────
  const setQueryParams = useCallback(
    (name: string, value: string) => {
      ...
      // 2. Update URL for bookmarkability
              ...
              <div className="flex flex-wrap gap-2">
                {selectedCategories.map((h) => (
                  <Pill
                    ...
                    onRemove={() =>
                      setQueryParams("categories", selectedCategories.filter((x) => x !== h).join(","))
                    }
                  />
                ))}
