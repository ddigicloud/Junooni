...
// CollectionTemplate.tsx - CLIENT-SIDE FILTERING (instant, no server round-trips)
import { Suspense } from "react"
import { SortOptions } from "@modules/store/components/refinement-list/sort-products"
import MobileStoreWrapper from "@modules/store/components/mobile-store-wrapper"
import { HttpTypes } from "@medusajs/types"
...
import LocalizedClientLink from "@modules/common/components/localized-client-link"
import { notFound } from "next/navigation"
import ClientCollectionShell from "../components/client-collection-shell"
  ...
  return (
    <div className="flex flex-col px-0 py-0 small:py-6 small:px-6 small:flex-row small:items-start content-container">
      ...
      <div className="w-full ml-0 md:ml-4">
        ...
        <div className="w-64 h-4 mb-6 bg-gray-100 rounded animate-pulse" />
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-8">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="flex flex-col gap-2">
              <div className="w-full aspect-[3/4] bg-gray-100 rounded-lg animate-pulse" style={{ animationDelay: `${i * 40}ms` }} />
...
async function CollectionDataFetcher({
  collection,
  countryCode,
  sortBy,
  initialCategoryHandles,
  initialVendorNames,
  initialColors,
  initialMinPrice,
  initialMaxPrice,
  ...
  const [categoriesData, vendorsData, { response: { products: allProducts } }] =
    await Promise.all([
      ...
      listProductsWithSort({
        ...
        sortBy: "created_at", // always fetch default order — client handles sorting
        countryCode,
      }),
    ])

  return (
    <ClientCollectionShell
      ...
      initialCategoryHandles={initialCategoryHandles}
      initialVendorNames={initialVendorNames}
...
export default async function CollectionTemplate({
  ...
  // Parse URL params — passed as initial state to ClientCollectionShell
  const initialCategoryHandles = categoryHandle
    ? categoryHandle.split(',').map(c => c.trim()).filter(Boolean)
    : []

  const initialVendorNames = vendors
  ...
  if (price) {
    const [min, max] = price.split('-').map(p => parseInt(p, 10))
    if (!isNaN(min)) initialMinPrice = min
    if (!isNaN(max)) initialMaxPrice = max
  }

  const hasAnyFilters =
    initialCategoryHandles.length > 0 ||
    initialVendorNames.length > 0 ||
  ...
  return (
    ...
    >
      {/*
        Suspense shows skeleton on FIRST load only.
        After CollectionDataFetcher resolves → ClientCollectionShell takes over.
        All filter/sort/page actions after that are 100% client-side = instant.
      */}
      <Suspense fallback={<CollectionLoadingSkeleton />}>
        <CollectionDataFetcher
          collection={collection}
          countryCode={countryCode}
          sortBy={sort}
          initialCategoryHandles={initialCategoryHandles}
