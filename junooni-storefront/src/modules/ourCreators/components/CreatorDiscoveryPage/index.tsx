      ...
      await Promise.all(
        creators.map(async (creator) => {
          if (creator.vendor && creator.vendor.id) {
            const result = await retriveVendorsFollowers(creator.vendor.id) as FollowerResult;
            
            // If there's data and it has a followers array or count
            if (result) {
              // Assuming the API returns an object with follower data
              // Store the count or 0 if no followers
