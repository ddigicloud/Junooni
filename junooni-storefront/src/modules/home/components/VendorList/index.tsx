...
// ─── Swiper — direct imports (dynamic split was causing layout breakage) ──────
import { Swiper, SwiperSlide } from "swiper/react";
import "swiper/css";
import "swiper/css/pagination";
import "swiper/css/navigation";
...
interface VendorListProps {
  // ✅ FIX 1: Accept vendors as a prop — data already fetched server-side in page.tsx
  vendorsList: Vendor[];
}
...
// ─── Swiper breakpoints (extracted to avoid repetition) ──────────────────────
...
const VendorList: React.FC<VendorListProps> = ({ vendorsList }) => {
  ...
  // ✅ FIX 1 cont: Filter featured vendors synchronously — no useEffect, no fetch, no skeleton
  ...
  return (
    <section className="relative py-20 bg-gray-50">
      <div className="container px-2 mx-auto sm:px-5">
        ...
        <div className="relative -mx-2 overflow-hidden sm:mx-0">
          <div className="px-2 sm:px-0">
            {/* ✅ FIX 2: Dynamic Swiper — no skeleton needed, data is ready immediately */}
            <Swiper
              slidesPerView={1.2}
              spaceBetween={16}
              navigation={{
                    ...
                    <div className="relative w-full mx-auto mb-3 overflow-hidden transition-all duration-300 bg-gray-100 shadow-sm aspect-square group-hover:shadow-md">
                      {vendor.logo ? (
                        <Image
                          ...
                          alt={vendor.name}
                          fill
                          // ✅ FIX 3: Removed unoptimized={true} — Next.js now serves WebP at correct size
                          // ✅ FIX 3: sizes tells Next.js exactly what width to serve per breakpoint
                          sizes="(max-width: 640px) 50vw, (max-width: 1024px) 25vw, 20vw"
                          // ✅ FIX 4: First 3 slides load eagerly (LCP), rest lazy
                          priority={index < 3}
