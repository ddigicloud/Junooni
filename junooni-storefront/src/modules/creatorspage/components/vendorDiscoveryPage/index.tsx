...
const VendorsDiscoveryPage = () => {
      ...
      <Link href={`/creator/${handle}`}>
        ...
        >
          ...
          <div className="relative h-24 bg-gradient-to-br from-orange-400 to-orange-600">
            {coverphoto && (
              <img
                src={typeof coverphoto === 'string' ? coverphoto : coverphoto.src}
                alt={name}
          ...
          <div className="absolute top-14 left-4">
            <div className="w-16 h-16 overflow-hidden bg-white border-4 border-white rounded-full shadow-md">
              <img
                src={
                  typeof (logo || avatarImage) === 'string'
                    ? (logo || avatarImage)
                    : (logo || avatarImage)?.src || ''
                }
                alt={name}
                className="object-cover w-full h-full"
              />
  ...
  return (
   ...
   bg-white border-t border-b shadow-smm">Filter by Category</h3>
  container px-4 py-4 mx-auto          className="text-sm texflex items-center justify-between mb-3        onClick={() => {
       font-mediumsetActiveCategory("all")
                  setSearchQuery("")
                }}
              >
                Reset Filters
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map((category) => (
                <button
                  key={category.id}
                  className={`flex itflex flex-wrap gap-2.5 rounded-full text-sm font-medium transition ${
      ...
      <main className="container px-4 py-6 mx-auto">
        {isLoading ? (
          ...
          <>
            {/* Vendors Grid container px-4 py-6 mx-autoName="grid grid-cols-1 gap-4 mb-8 md:grid-cols-2 lg:grid-cols-4">
              {currentVendors.map((vendor) => (
                <VendorCard key={vendor.id} vendor={vendor} grid grid-cols-1 gap-4 mb-8 md:grid-cols-2 lg:grid-cols-4{/* Pagination */}
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-8">
                {/* Previous Button */}
                <button
                  onClick={handlePrevPage}
             flex items-center justify-center gap-2 mt-8          className={`flex items-center gap-1 px-3 py-2 rounded-lg border transition ${
                    currentPage === 1
         ...
         hidden sm:inlineeNumbers().map((pageNum, index) => (
                    pageNum === '...' ? (
                      <span key={`ellflex items-center gap-1ame="px-2 text-gray-400">
                        ...
                      </span>
                    ) : (
                      <button
                        key={ppx-2 text-gray-400               onClick={() => handlePageChange(pageNum as number)}
            ...
            {/* Results inhidden sm:inline    <div className="mt-4 text-sm text-center text-gray-600">
              Showing {indexOfFirstVendor + 1} to {Math.min(indexOfLastVendor, filteredVendors.length)} of {filteredVendors.lengmt-4 text-sm text-center text-gray-600     </>
        ) : (
          // Empty state
          <div className="p-8 text-center bg-white rounded-lg shadow-sm">
            <div className="mb-4 text-gray-400">
              <Search size={64} className="mx-auto" />
            </dp-8 text-center bg-white rounded-lg shadow-smfont-medium">No vendors found</hmb-4 text-gray-400 className="mb-6 text-gray-600">
             mx-autohQuery || activeCategory !== 'all'
                ?mb-2 text-xl font-mediumrch terms or filters"
                : "No vendormb-6 text-gray-600 the moment"}
            </p>
