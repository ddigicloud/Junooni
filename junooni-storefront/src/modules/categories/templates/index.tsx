...
// CategoryTemplate.tsx - CLIENT-SIDE FILTERING (instant, no server round-trips)
import { Suspense } from "react"
import { notFound } from "next/navigation"
import { SortOptions } from "@modules/store/components/refinement-list/sort-products"
import MobileStoreWrapper from "@modules/store/components/mobile-store-wrapper"
...
import { listProductsWithSort } from "@lib/data/products"
import { getRegion } from "@lib/data/regions"
import ClientCategoryShell from "../components/client-category-shell"
...
async function CategoryDataFetcher({
  ...
  // ONE fetch — all products in this category, default sort, client handles rest
  ...
  return (
    <ClientCategoryShell
      ...
      initialCollectionHandles={initialCollectionHandles}
...
export default async function CategoryTemplate({
  ...
  // Parse URL params → initial state for ClientCategoryShell
  ...
  const initialCollectionHandles = selectedCollections
    ? selectedCollections.split(",").map((c) => c.trim()).filter(Boolean)
    : []
  ...
  const hasAnyFilters =
    ...
    initialCollectionHandles.length > 0 ||
