...
const WishlistButton: React.FC<WishlistButtonProps> = ({
  ...
  const { isInWishlist, addToWishlist, removeFromWishlist, isLoggedIn, isLoading } =
    useWishlist()
  const router = useRouter()
  const [isPending, setIsPending] = React.useState(false)

  // No useEffect, no individual API calls — reads from shared context
  const inWishlist = variantId ? isInWishlist(variantId) : false

  const handleClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault()
    e.stopPropagation()

    if (!variantId || isPending || isLoading) return
    ...
    try {
      if (isWishlistPage) {
        if (onRemove) onRemove(variantId)
  ...
  return (
    ...
    >
      {isWishlistPage ? (
        <X className="w-5 h-5 text-gray-700 transition-all scale-75 rounded hover:text-red-500 hover:scale-100 hover:bg-gray-100" />
      ) : (
        <Heart
          className={`w-5 h-5 transition-all ${
            inWishlist
