...
// ─── ClientStoreShell ─────────────────────────────────────────────────────────
// Receives ALL products from the server once on first load.
// All filtering / sorting / pagination happens in the browser — zero server calls.
export default function ClientStoreShell({
  ...
  // ── Push URL for bookmarkability — does NOT re-render server ──────────────
  ...
  // ── Sidebar data — computed once from all products ─────────────────────────
  ...
  // Updates state IMMEDIATELY (instant UI), then pushes URL (bookmarkability).
  const setQueryParams = useCallback(
    (name: string, value: string) => {
      // 1. Instant state update → triggers re-filter in useMemo above
      switch (name) {
        case "sortBy":
        ...
        case "page":
          ...
          return // page change doesn't reset page, skip below
      }
      // Reset to page 1 on any filter/sort change
      setCurrentPage(1)

      // 2. Push URL for bookmarkability (doesn't block UI)
      const params = new URLSearchParams(searchParams.toString())
  ...
  // ── Page change handler — passed to Pagination via PaginatedProductsDisplay
    ...
    <div className="flex flex-col px-0 py-0 small:py-6 small:px-6 small:flex-row small:items-start content-container">
      ...
      <div className="flex-shrink-0 hidden md:block">
        <RefinementList
          ...
          selectedCategories={selectedCategories}  // ← is this here?
