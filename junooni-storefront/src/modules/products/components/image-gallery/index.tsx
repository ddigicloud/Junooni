...
type ImageGalleryProps = {
  images: HttpTypes.StoreProductImage[];
  selectedVariantId?: string; // ID of the currently selected variant
};

const ImageGallery = ({ images, selectedVariantId }: ImageGalleryProps) => {
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const [selectedImageIndex, setSelectedImageIndex] = useState<number | null>(null);
  ...
  const filteredImages = selectedVariantId 
    ? images.filter(image => {
        // Check if image has variant_id metadata that matches selected variant
        const metadata = image.metadata as { variant_id?: string } | null;
        return metadata?.variant_id === selectedVariantId;
      })
    : images;
  
  // Use all images if no variant-specific images found
  const displayImages = filteredImages.length > 0 ? filteredImages : images;
  
  // Reset current image index when variant changes
  useEffect(() => {
