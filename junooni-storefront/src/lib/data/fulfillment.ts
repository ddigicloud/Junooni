...
export const addShippingMethod = async ({
  ...
  return sdk.client
    .fetch<HttpTypes.StoreCartResponse>(`/store/carts/${cartId}/shipping-methods`, {
      method: "POST",
      body: { option_id: optionId, data: data ?? {} },
      headers,
    })
    .then(({ cart }) => cart)
    .catch((e) => {
      console.error("Failed to add shipping method:", e)
      return null
    })
}

export const removeShippingMethod = async ({
  cartId,
  shippingMethodId,
}: {
  cartId: string
  ...
  return sdk.client
    ...
    .then(({ cart }) => cart)
    .catch((e) => {
      console.error("Failed to remove shipping method:", e)
      return null
    })
}